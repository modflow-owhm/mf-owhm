...
C
      ...
      ENDIF
      IF ( INRCH.NE.Z ) CALL CFP1RCH1RP(INRCH)
...
C
 ...
 100  WRITE (IOUT, *) ' FILE TYPE "CRCH" NOT FOUND, EOL, OR PROTECTED!'
      RETURN
 200  CALL STOP_ERROR(INFILE=INUNIT, OUTPUT=IOUT, 
     +                MSG="CRCH INPUT READ ERROR."//NL//
     +                    "COMMENT LINE EXPECTED."//NL//
     +                    "INSTEAD READ: "//NL//TRIM(CDUMMY)//NL)
!200  WRITE (IOUT, *) ' READ ERROR CONDUIT INPUT FILE. WRONG DATA TYPE',
!    +                'COMMENT LINE EXPECTED, INSTEAD READ: ', CDUMMY
!     STOP
 300  CALL STOP_ERROR(INFILE=INUNIT, OUTPUT=IOUT, GUESS=.TRUE.,
     +                MSG="CRCH INPUT READ ERROR"//NL//
     +                    "LAST COMMENT LINE: "//NL//TRIM(CDUMMY)//NL)
! 300  WRITE (IOUT, *) ' READ ERROR CONDUIT INPUT FILE. WRONG DATA TYPE',
!     +                'DATA LINE EXPECTED. LAST COMMENT LINE: ', CDUMMY
!      STOP
 9001 FORMAT (15(G7.2E1,1x))
C
