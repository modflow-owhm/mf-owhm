...
MODULE PARSE_WORD_INTERFACE!, ONLY: PARSE_WORD, PARSE_WORD_UP, FIND_NONBLANK
  ...
  INTEGER,   PARAMETER:: ASCII_DIFF = ICHAR('a')-ICHAR('A')
  ! 
  ! ----------------------------------------------------------------------------------------
  ! 
  CONTAINS
  ...
  PURE SUBROUTINE PARSE_WORD_UP(LN, LOC, ISTART, ISTOP, COM_STOP, FIND_NEXT, OLD_LOC, EOL)
    ...
    IF(ISTART.LE.ISTOP) THEN
        DO CONCURRENT(I=ISTART:ISTOP, LN(I:I).GE.'a' .AND. LN(I:I).LE.'z')
                                                                          LN(I:I)=CHAR(ICHAR(LN(I:I))-ASCII_DIFF)  !Make parsed section upper case
        END DO
    END IF
    !
  END SUBROUTINE
  ...
  !  Get Word Routines
  !  
  FUNCTION GET_WORD_FUNCTION(LN,LOC,COM_STOP,NO_UPCASE) RESULT(WORD) ! SETS WORD TO PARSED WORD, SETS IT TO "" IF END OF LINE OR NO WORD PARSED
    CHARACTER(*),             INTENT(IN):: LN          ! LINE TO PARSE WORD FROM
  ...
  !#############################################################################################################################################################
  !
  FUNCTION GET_WORD_FUNCTION_N(N,LN,COM_STOP,NO_UPCASE) RESULT(WORD) ! SETS WORD TO PARSED WORD, SETS IT TO "" IF END OF LINE OR NO WORD PARSED
  ...
  PURE SUBROUTINE UPPER(LN)               !UPPER only for internal use to MODULE PARSE_WORD_INTERFACE --> coerce compiler to inline calls in module 
    CHARACTER(*), INTENT(INOUT):: LN
    INTEGER::I
    DO CONCURRENT(I=1:LEN(LN), LN(I:I).GE.'a' .AND. LN(I:I).LE.'z');  LN(I:I)=CHAR(ICHAR(LN(I:I))-ASCII_DIFF)
    END DO
  END SUBROUTINE
  !
  ! -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
