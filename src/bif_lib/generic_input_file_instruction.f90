...
MODULE GENERIC_INPUT_FILE_INSTRUCTION!, ONLY: GENERIC_INPUT_FILE                        --side note, reposition STREAM with READ(IU,'()', ADVANCE='NO', POS=P)
  ...
  CONTAINS
  !
  RECURSIVE SUBROUTINE OPEN_GENERIC_INPUT_FILE(FL,LN,LLOC,OUTPUT,INFILE,NOSTOP,REQKEY,NOSFAC,IU,BINARY,BUFFER,EOL,NO_INTERNAL,NO_CONSTANT,NEW_UNIT,SAVE_FNAME,KEY,DIM,PREPOST,STREAM_TEXT,NOPOSTKEY,FORCE_DATAFILE,ONLY_CHECK,MSG)
    ...
    LOGICAL,     OPTIONAL,      INTENT(IN   ):: ONLY_CHECK
    ...
    IF (IERR .NE. Z) THEN
          ...
          IF(EXT == 'INTERNAL') THEN
                                           FOUND_KEY = TRUE
                                           FL%IU = Z
                                           IF(CHECK_POST) CALL CHECK_FOR_POST_KEY(LL,LN,IIN,FL%IOUT,BUF,SPLIT,FL%SCALE,GO_TO_TOP=GO_TO_TOP,DIM=DIM,OLDLOC=PREPOST, ONLY_CHECK=ONLY_CHECK, MSG=MSG)
          ...
          ELSEIF(EXT == 'SKIP' .OR. EXT =='NAN' .OR. EXT =='NULL' .OR. EXT =='NUL') THEN
                                           ...
                                           FL%ERROR  = TRUE
                                           FL%SKIP   = TRUE
                                           IF(CHECK_POST) CALL CHECK_FOR_POST_KEY(LL,LN,IIN,FL%IOUT,BUF,SPLIT,FL%SCALE,DIM=DIM,OLDLOC=PREPOST, ONLY_CHECK=ONLY_CHECK, MSG=MSG)
          ...
          ELSE
                ...
                IF(EXIST) THEN
                    !
                    IF(CHECK_POST) CALL CHECK_FOR_POST_KEY(LL,LN,IIN,FL%IOUT,BUF,SPLIT,FL%SCALE,BINARY=FL%BINARY,GO_TO_TOP=GO_TO_TOP,DIM=DIM,OLDLOC=PREPOST, ONLY_CHECK=ONLY_CHECK, MSG=MSG) !FL%BINARY ONLY SET TO TRUE IF BINARY FLAG FOUND, OTHERWISE IGNORED
                    !
