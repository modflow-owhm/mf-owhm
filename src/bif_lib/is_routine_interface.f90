...
MODULE IS_ROUTINES
  ...
  PURE FUNCTION IS_BLANK(LN) RESULT(EMPTY)
    ...
    DO I=ONE, LEN_TRIM(LN)
        IF(LN(I:I).NE." " .AND. LN(I:I).NE.TAB .AND. LN(I:I).NE."," ) THEN
            EMPTY = FALSE
            EXIT
  ...
  PURE FUNCTION STR_IS_UNIQUE(VAL, VEC, UPCASE) RESULT (UNI)
    ...
    ELSE
       ...
       BLOCK
         ...
         !
         SUB = VAL
         DO CONCURRENT(K=1:N, SUB(K:K).GE.'a' .AND. SUB(K:K).LE.'z');  SUB(K:K)=CHAR(ICHAR(SUB(K:K))-ASCII_DIFF)
         END DO
         !
         DO I=ONE, DIM
             UNI = TRUE
             !
             STR = VEC(I)
             DO CONCURRENT(K=1:M, STR(K:K).GE.'a' .AND. STR(K:K).LE.'z');  STR(K:K)=CHAR(ICHAR(STR(K:K))-ASCII_DIFF)
             END DO
             !
             IF(SUB==STR) THEN
                              UNI=FALSE
