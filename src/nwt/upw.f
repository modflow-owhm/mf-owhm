...
!
     ...
     2                     LBOTM,HNEW, CBC_GLOBAL_UNIT
      ...
      USE CONSTANTS, ONLY: NL, BLN, FALSE, TRUE, DZ, UNO, Z, ONE
      IMPLICIT NONE
!     ------------------------------------------------------------------
!     SPECIFICATIONS:
!     ------------------------------------------------------------------
...
!     LOCAL VARIABLES FOR DEFINING CELL PROPERTES (FROM LPF)
	...
	TYPNAM(2) = '   CONVERTIBLE'
      ...
      HANNAM = '      VARIABLE'
      !
      ANAME(1) = '   HYD. COND. ALONG ROWS'
      ANAME(2) = '  HORIZ. ANI. (COL./ROW)'
      ANAME(3) = '     VERTICAL HYD. COND.'
      ANAME(4) = ' HORIZ. TO VERTICAL ANI.'
      ANAME(5) = 'QUASI3D VERT. HYD. COND.'
      ANAME(6) = '        SPECIFIC STORAGE'
      ANAME(7) = '          SPECIFIC YIELD'
      ANAME(8) = '        WETDRY PARAMETER'
      ANAME(9) = '     STORAGE COEFFICIENT'
      !
!     ------------------------------------------------------------------
...
!     CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IPHDRY,R,0,IN)
     ...
     +                 HAS_ERROR=CHK)
      ...
      CALL CHECK_CBC_GLOBAL_UNIT(IUPWCB, .TRUE.)
      !
!
C
C3A-----WRITE ITEM 1
        IF(IUPWCB < Z) WRITE(IOUT,8)
    8 FORMAT(1X,'CONSTANT-HEAD CELL-BY-CELL FLOWS WILL BE PRINTED',
     1  ' WHEN ICBCFL IS NOT 0')
        IF(IUPWCB > Z) WRITE(IOUT,9) IUPWCB
...
C3B-----GET OPTIONS.
   ...
   21      FORMAT(1X,'STORAGECOEFFICIENT OPTION:',/,
     ...
     2       1X,'Option not supported when using ',
     3          'UPW Flow Package and NWT Solver')
        CASE('CONSTANTCV')
           ICONCV = ONE
           WRITE(IOUT,23)
   23      FORMAT(1X,'CONSTANTCV OPTION:',/,1X,'Constant vertical',
     ...
     3            'UPW Flow Package and NWT Solver')
        CASE('THICKSTRT')
           ITHFLG = ONE
           WRITE(IOUT,25)
   25      FORMAT(1X,'THICKSTRT OPTION:',/,1X,
     1            'Negative LAYTYP indicates confined layer ',
     2            'with thickness computed from STRT-BOT',
     3         1X,'Option not supported when using ',
     4            'UPW Flow Package and NWT Solver')
        CASE('NOCVCORRECTION')
           NOCVCO = ONE
           WRITE(IOUT,27)
   ...
   29      FORMAT(1X,'NOVFC OPTION:',/,1X,
     1     'vertical flow correction not supported when using ',
     2     'UPW Flow Package and NWT Solver')
        CASE('NOPARCHECK')
           NOPCHK = ONE
           WRITE(IOUT,30)
   30      FORMAT(1X,'NOPARCHECK  OPTION:',/,1X,
     ...
     2        'parameters define data at all cells')
        CASE('LPF_STORAGE','LPF')
              IF(LINE(ISTART:ISTOP) == 'LPF') 
     +                       CALL PARSE_WORD(LINE,LLOC,ISTART,ISTOP)
              !
              LPF_STORAGE = TRUE
              WRITE(IOUT,'(A)') ' LPF_STORAGE OPTION ENABLED'
        CASE DEFAULT
                     EXIT
        END SELECT
        !
...
C4------ALLOCATE AND READ LAYTYP, LAYAVG, CHANI, LAYVKA, LAYWET, LAYSTRT.
      ...
      IF(LINE(ISTART:ISTOP) == 'CONVERTIBLE') THEN
        ...
        ELSE
            LAYTYPUPW = ONE
        END IF
      ELSEIF(LINE(ISTART:ISTOP) == 'CONFINED' .OR.
     +       LINE(ISTART:ISTOP) == 'CONFINE' ) THEN
      LAYTYPUPW = Z
      ELSE
          BACKSPACE(IN)
      ...
      LLOC=1
      CALL PARSE_WORD_UP(LINE,LLOC,ISTART,ISTOP)
      IF(LINE(ISTART:ISTOP) == 'HARMONIC') THEN
         LAYAVG = 0
      ELSEIF(LINE(ISTART:ISTOP) == 'LOGARITHMIC') THEN
         LAYAVG = 1
      ELSEIF(LINE(ISTART:ISTOP) == 'ARITHMETIC') THEN
         LAYAVG = 2
      ELSE
          BACKSPACE(IN)
     ...
     +   LINE(ISTART:ISTOP) == 'NO'      ) THEN
      ...
      LLOC=1
      CALL PARSE_WORD_UP(LINE,LLOC,ISTART,ISTOP)
      IF(LINE(ISTART:ISTOP) == 'VKA') THEN
          LAYVKAUPW = Z
      ELSEIF(LINE(ISTART:ISTOP) == 'RATIO') THEN
          LAYVKAUPW = ONE
      ELSE
          BACKSPACE(IN)
      ...
      LLOC=1
      CALL PARSE_WORD_UP(LINE,LLOC,ISTART,ISTOP)
      IF(LINE(ISTART:ISTOP) == 'WET'.OR.
     +   LINE(ISTART:ISTOP) == 'WETTING') THEN
         CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,K,ERROR_VAL=0)
         IF(K > Z) THEN
            LAYWET     = Z
            LAYWET(:K) = ONE
     ...
     +       LINE(ISTART:ISTOP) == 'NO_WETTING') THEN
      ...
      END IF
      !
      IF(ANY(LAYWET /= Z)) THEN
        CALL WARNING_MESSAGE(LINE,IN,IOUT,
     +  MSG='UPW DOES NOT SUPPORT WETTING.'//NL//
     + 'Because cells never go dry in UPW.'//BLN//
     + 'Any input pertaining to WETTING is read, but ignored.')
      END IF
C
C4A-----PRINT A TABLE OF VALUES FOR LAYTYP, LAYAVG, CHANI, LAYVKA, LAYWET.
   ...
   47 FORMAT(1X,/3X,'LAYER FLAGS:',/1X,
     ...
     2 '       LAYVKA        LAYWET',/1X,75('-'))
      DO 50 K=1,NLAY
        WRITE(IOUT,48)K,LAYTYPUPW(K),LAYAVG(K),CHANI(K),LAYVKAUPW(K),0
   48 FORMAT(1X,I4,2I14,ES14.3,2I14)
C
C4A1----SET GLOBAL HEAD-DEPENDENT TRANSMISSIVITY AND STORAGE FLAGS.
      IF (LAYTYPUPW(K) > Z) THEN
        LAYHDT(K) = ONE
        LAYHDS(K) = ONE
      ELSE
        LAYHDT(K) = Z
...
C4A2----SET LAYSTRT AND RESET LAYTYP IF THICKSTRT OPTION IS ACTIVE.
      DO 60 K=1,NLAY
      LAYSTRT(K) = Z
      IF(LAYTYPUPW(K) < Z .AND. ITHFLG /= Z) THEN
         LAYSTRT(K)   = ONE
         LAYTYPUPW(K) = Z
         LAYHDT(K)    = Z
         LAYHDS(K)    = Z
...
C4B-----TO SC2, HANI, and WETDRY.  PRINT INTERPRETED VALUES OF FLAGS.
   ...
   67 FORMAT(1X,/3X,'INTERPRETATION OF LAYER FLAGS:',/1X,
     ...
     6  '      (LAYVKA)      (LAYWET)',/1X,75('-'))
      ...
      IF(LAYWET(K) /= Z) THEN
         IF(LAYTYPUPW(K) == Z) THEN
            CALL USTOP(' LAYWET is not 0 and LAYTYP is 0 for layer: '//
     +      NUM2STR(K)//NL//'LAYWET must be 0 if LAYTYP is 0')
         ELSE
            NWETD=NWETD+1
            LAYWET(K)=NWETD
         END IF
      END IF
      IF(LAYAVG(K) < Z .OR. LAYAVG(K) > 2) THEN
   ...
   74    FORMAT(1X,I8,
     1    ' IS AN INVALID LAYAVG VALUE -- MUST BE 0, 1, or 2')
      ...
      END IF
      LAYPRN(1)=TYPNAM(1)
      IF(LAYTYPUPW(K) > Z) LAYPRN(1)=TYPNAM(2)
      ...
      END IF
      LAYPRN(4)=VKANAM(1)
      IF(LAYVKAUPW(K) /= Z) LAYPRN(4)=VKANAM(2)
      !
      IF(LAYTYPUPW(K) == Z) THEN
          LAYPRN(5)=WETNAM(1)
...
C4C-----PRINT WETTING INFORMATION.  RGN commented out because this does not apply
      IWDFLG = Z
      IF(NWETD>Z) CALL READ_TO_DATA(LINE,IN,IOUT)  ! ByPass: READ(IN,*) WETFCT,IWETIT,IHDWET
...
C5------ALLOCATE MEMORY FOR ARRAYS.
      ...
      END IF
      SC1 = DZ
! RGN 6/25/09      IF(ITRSS /= Z .AND. NCNVRT > Z) THEN
      IF(ITRSS /= Z) THEN
...
C6A-----MUST BE DEFINED USING PARAMETERS.  ENSURE THAT ALL CHANI <= 0
  ...
  118       CONTINUE
         ...
         ELSE
              WRITE(IOUT,"(A)")
     +     ' UPW Package bad parameter PARTYP for : "'//PTYP//'"'//NL//
...
C7A-----DEFINE HORIZONTAL HYDRAULIC CONDUCTIVITY (HK)
     ...
     +            MSG='UPW failed to read HK print code (IPRN)')
         !
...
C7B-----READ HORIZONTAL ANISOTROPY IF CHANI IS NON-ZERO
     ...
     +            MSG='UPW failed to read HANI print code (IPRN)')
         !
...
C7C-----ANISOTROPY (VKA).
     ...
     +            MSG='UPW failed to read VKA print code (IPRN)')
         !
...
C7D-----DEFINE SPECIFIC STORAGE OR STORAGE COEFFICIENT IN ARRAY SC1 IF TRANSIENT.
     ...
     +               MSG='UPW failed to read Ss print code (IPRN)')
            !
...
C7E-----IS CONVERTIBLE.
     ...
     +                  MSG='UPW failed to read Sy print code (IPRN)')
               !
     ...
     +                   NCOL,NLAY,NROW,'SY  ')
         ...
         END IF
      !ELSE
      !  IF(ITRSS /= Z) THEN  ! already set to zero at initialization.
      !    DO J=1,NROW
      !      DO I=1,NCOL
...
C7F-----READ CONFINING BED VERTICAL HYDRAULIC CONDUCTIVITY (VKCB)
     ...
     +                  MSG='UPW failed to read VKCB print code (IPRN)')
            !
...
C7G-----(LAYWET NOT 0).
      IF(LAYWET(K) /= Z) THEN
         CALL U2DREL(WETDRY(:,:,1),ANAME(8),NROW,NCOL,KK,IN,
     1            IOUT)
      END IF
  200 CONTINUE
C
      IF(NWETD > Z) THEN  !Disable Wetting if enabled
         DEALLOCATE(WETDRY)
         ALLOCATE(WETDRY(1,1,1))
         LAYWET = Z
         NWETD  = Z
...
C2------TRANSMISSIVE PARAMETER.
      DO 60 K=1,NLAY
         DO  I=1,NROW
         DO  J=1,NCOL
         IF(IBOUND(J,I,K).EQ.0 ) CYCLE !GO TO 40
C
C3A-----CHECK HORIZONTAL HYDRAULIC CONDUCTIVITY (HK).
         IF(HKUPW(J,I,K).NE.ZERO) CYCLE !GO TO 40
C
...
C3B-----VERTICAL HYDRAULIC CONDUCTIVITY.
         IF(NLAY.GT.1) THEN
            ...
            IF(VKAUPW(J,I,K).NE.ZERO) THEN
               IF(K.NE.NLAY) THEN
                  IF (VKAUPW(J,I,K+1).NE.ZERO) CYCLE !GO TO 40
               END IF
               IF(K.NE.1) THEN
                  IF (VKAUPW(J,I,K-1).NE.ZERO) CYCLE !GO TO 40
               END IF
...
C
   60 CONTINUE
      ...
      SUBROUTINE GWF2UPWFMS(KITER,KSTP,KPER,IGRID)
         USE GWFUPWMODULE, ONLY: LPF_STORAGE
         IMPLICIT NONE
         INTEGER, INTENT(IN):: KITER,KSTP,KPER,IGRID
        
         IF(LPF_STORAGE) THEN
             CALL GWF2UPWFMS_LPF(KITER,KSTP,KPER,IGRID)
         ELSE
             CALL GWF2UPWFMS_UPW(KITER,KSTP,KPER,IGRID)
         END IF
      END SUBROUTINE
      !
      !
      SUBROUTINE GWF2UPWFMS_LPF(KITER,KSTP,KPER,IGRID)
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     1                      LBOTM,CV,HNEW,RHS,HCOF,HOLD,ISSFLG,IOUT
      ...
      USE GWFUPWMODULE
      USE CONSTANTS, ONLY: DZ, UNO, Z, ONE
      IMPLICIT NONE
      INTEGER, INTENT(IN):: KITER,KSTP,KPER,IGRID
      DOUBLE PRECISION, EXTERNAL :: DHORIZUPW
      DOUBLE PRECISION:: HTMP, TP, BT, TLED, SOLD, SNEW, STRG
      DOUBLE PRECISION:: RHO1, RHO2, HLD, THICK
      INTEGER:: I,K,J
      INTEGER:: iSY, id, iTop, iBot
C     ------------------------------------------------------------------
C
...
C3------WHEN TO USE PRIMARY AND SECONDARY STORAGE
         ...
         !
         IF(iSY == Z) THEN
           !dS = DZ -> Confine layers have no derivative
           DO I=1,NROW
           DO J=1,NCOL
             IF(IBOUND(J,I,K) <= Z) CYCLE
             !
             TP = BOTM(J, I, iTop)
             BT = BOTM(J, I, iBot)
             THICK = TP-BT
             !
             RHO1 = SC1(J,I,K)*THICK*TLED
             !
             HCOF(J,I,K) = HCOF(J,I,K) - RHO1
             RHS(J,I,K)  = RHS(J,I,K)  - RHO1*HOLD(J,I,K)
             !
           END DO
           END DO
         ELSE
           ...
           DO J=1,NCOL
             ...
             HLD  = HOLD(J,I,K)
             !
             IF(HTMP < BT .AND. HLD < BT) CYCLE    ! Cell is dry nothing to do
             !
             RHO1 = SC1   (J,I,  K)*TLED*THICK
             RHO2 = SC2UPW(J,I,iSY)*TLED
             !                !
             IF(HLD  >= TP) THEN
                            SOLD=RHO1
             ELSE
             ...
             END IF
             !
             !!!IF(HTMP >= TP .AND. HLD >= TP) THEN   ! dS = DZ -> Fully-Saturated layers have no derivative
             !!!   !
             !!!   HCOF(J,I,K) = HCOF(J,I,K) - RHO1
             !!!   RHS(J,I,K)  = RHS(J,I,K)  - RHO1*HLD
             !!!   !
             !!!ELSEIF(HTMP <= TP .AND. HLD <= TP .AND.
     +       !!!       HTMP >= BT .AND. HLD >= BT) THEN    ! Water Table Condition
             !!!   !
             !!!   HCOF(J,I,K) = HCOF(J,I,K) - RHO2
             !!!   RHS(J,I,K)  = RHS(J,I,K)  - RHO2*HLD
             !!!   !
             !!!ELSE
             IF(HTMP >= BT .AND. HLD >= BT) THEN   !Not Dry, but mixed (MF-2005 formulation) -- Note this could replace the previous to ELSEIFs

                !
                HCOF(J,I,K) = HCOF(J,I,K) - SNEW
                RHS(J,I,K)  = RHS(J,I,K)  - SOLD*(HLD-TP) - SNEW*TP
                !
             ELSEIF(HLD < BT) THEN  ! .AND.  HTMP >= BT 
                !
                HLD = BT
                HCOF(J,I,K) = HCOF(J,I,K) - SNEW
                RHS(J,I,K)  = RHS(J,I,K)  - SOLD*(HLD-TP) - SNEW*TP  ! Note that (HLD-TP) = -THICK
                !
             ELSE! IF(HTMP < BT .AND. HLD >= BT) THEN 
                !
                !HTMP = BT
                !HCOF(J,I,K) = HCOF(J,I,K) - SNEW  => Move to RHS
                RHS(J,I,K)  = RHS(J,I,K) - SOLD*(HLD-TP) - SNEW*TP 
     +                                                   + SNEW*BT
             END IF
             !
      ...
      !
      !
      SUBROUTINE GWF2UPWFMS_UPW(KITER,KSTP,KPER,IGRID)
C     ******************************************************************
C     ADD LEAKAGE CORRECTION AND STORAGE TO HCOF AND RHS, AND CALCULATE
C     CONDUCTANCE AS REQUIRED.
C     ******************************************************************
...
C4A-----IF THE CELL IS EXTERNAL THEN SKIP IT.
            IF(IBOUND(J,I,K).LE.0) CYCLE !GO TO 180
...
!      BUFF(J,I,K)=ZERO
!  310 CONTINUE
      DO CONCURRENT(K=K1:K2, I=I1:I2, J=J1:J2) 
      BUFF(J,I,K)=ZERO
      END DO
C
C3B-----FOR EACH CELL CALCULATE FLOW THRU RIGHT FACE & STORE IN BUFFER.
      ...
      DO J=J1,J2
      IF(ICHFLG.EQ.0) THEN
         IF((IBOUND(J,I,K).LE.0) .AND. (IBOUND(J+1,I,K).LE.0)) CYCLE !GO TO 400
         IF((IBOUND(J,I,K).EQ.0) .OR. (IBOUND(J+1,I,K).EQ.0))  CYCLE !GO TO 400
      ELSE
         IF((IBOUND(J,I,K).EQ.0) .OR. (IBOUND(J+1,I,K).EQ.0))  CYCLE ! GO TO 400
      END IF
...
C4B-----FOR EACH CELL CALCULATE FLOW THRU FRONT FACE & STORE IN BUFFER.
      ...
      DO J=J1,J2
      IF(ICHFLG.EQ.0) THEN
         IF((IBOUND(J,I,K).LE.0) .AND. (IBOUND(J,I+1,K).LE.0)) CYCLE !GO TO 500
         IF((IBOUND(J,I,K).EQ.0) .OR. (IBOUND(J,I+1,K).EQ.0))  CYCLE !GO TO 500
      ELSE
         IF((IBOUND(J,I,K).EQ.0) .OR. (IBOUND(J,I+1,K).EQ.0))  CYCLE !GO TO 500
      END IF
...
C5C-----RECORD CONTENTS OF BUFFER AND RETURN.
     ...
     1     NLAY,IOUT,DELT,PERTIM,TOTIM,IBOUND)
      ...
      SUBROUTINE GWF2UPWBDS(KSTP,KPER,IGRID)
         ...
         IF(LPF_STORAGE) THEN
             CALL GWF2UPWBDS_LPF(KSTP,KPER,IGRID)
         ELSE
             CALL GWF2UPWBDS_UPW(KSTP,KPER,IGRID)
         END IF
      END SUBROUTINE
      !
      !
      SUBROUTINE GWF2UPWBDS_LPF(KSTP,KPER,IGRID)
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     1                      BUFF,BOTM,LBOTM,IOUT
      ...
      USE CONSTANTS,   ONLY: Z, ONE, DZ, UNO, NEARZERO_20
      !
      IMPLICIT NONE
      !
      INTEGER, INTENT(IN):: KSTP, KPER, IGRID
      !
      CHARACTER(16):: TEXT
      INTEGER:: iSY, IBD, iBot, iTop
      INTEGER:: I,J,K,ij
      LOGICAL:: CNVT
      DOUBLE PRECISION:: STOIN, STOUT, HSING, THICK
      DOUBLE PRECISION:: STRG, SIN, SOUT
      DOUBLE PRECISION:: BT, TP, RHO1, RHO2, SOLD, SNEW, HLD, HSN, TLED
C
      DATA TEXT /'         STORAGE'/
C     ------------------------------------------------------------------
C
...
C1------INITIALIZE BUDGET ACCUMULATORS AND 1/DELT.
      ...
      STOUT = DZ
      !
      IF(DELT < NEARZERO_20)THEN                                       ! ADDED CHECK FOR WHEN DELT=0, THIS WILL SET ALL STORAGES TO ZERO
        TLED = DZ
      ELSE
        TLED = UNO/DBLE(DELT)
      END IF
...
C3------IF CELL-BY-CELL FLOWS WILL BE SAVED, SET FLAG IBD.
      IBD = Z
      IF(IUPWCB > Z) IBD=ICBCFL
      !
      DO K=1,NLAY
      CNVT = LAYTYPUPW(K) > Z
      DO I=1,NROW
      DO J=1,NCOL
        IF(IBOUND(J,I,K) > Z .AND. CNVT) THEN
                             IF(HNEW(J,I,K) > BOTM(J,I,LBOTM(K))) THEN
...
C2------IF STEADY STATE, STORAGE TERM IS ZERO
      IF(ISSFLG(KPER) /= Z) THEN                                      ! ADDED PRINT OUT OF BUDGET EVEN WHEN STEADY STATE OCCURS
       ...
       LayLoop: DO K=1,NLAY
         ...
         IF(iSY == Z) THEN
           ...
           DO J=1,NCOL
             ...
             RHO1 = SC1(J,I,K)*THICK*TLED
             !
             STRG = RHO1*HOLD(J,I,K) - RHO1*HNEW(J,I,K)
             !
             BUFF(J,I,K)=STRG
             !
             IF(STRG < DZ) THEN
         ...
         ELSE
           ...
           DO J=1,NCOL
             ...
             HLD = HOLD(J,I,K)
             !
             IF(HSN < BT .AND. HLD < BT) CYCLE    ! Cell is dry nothing to do
             !
             RHO1 = SC1   (J,I,  K)*TLED*THICK
             RHO2 = SC2UPW(J,I,iSY)*TLED
             !                !
             ...
             END IF
             !
             IF(HSN >= BT .AND. HLD >= BT) THEN   !Not Dry, but mixed (MF-2005 formulation) -- Note this could replace the previous to ELSEIFs
                !
                STRG = SOLD*(HLD-TP) + SNEW*TP - SNEW*HSN
                !
             ELSEIF(HLD < BT) THEN  ! .AND.  HSN >= BT 
                !
                STRG = SOLD*(BT-TP) + SNEW*TP - SNEW*HSN
                !
             ELSE! IF(HSN < BT .AND. HLD >= BT) THEN 
                !
                !HSN = BT
                STRG = SOLD*(HLD-TP) + SNEW*TP - SNEW*BT
             END IF
             !
             BUFF(J,I,K)=STRG
             !
...
C10-----ADD TOTAL RATES AND VOLUMES TO VBVL & PUT TITLE IN VBNM.
  400 CONTINUE
      ...
      !
      !
      SUBROUTINE GWF2UPWBDS_UPW(KSTP,KPER,IGRID)
C     ******************************************************************
C     COMPUTE STORAGE BUDGET FLOW TERM.
C     ******************************************************************
C
...
C------Set old saturation to new saturation.
      ...
      END SUBROUTINE
      !
      !          GWF2UPWUPDATE. UPDATE VALUES AFTER OUTER ITERATION.
      ...
      USE SET_ARRAY_INTERFACE, ONLY: SET_ARRAY, SET_ZERO
      IMPLICIT NONE
!     ------------------------------------------------------------------
!     SPECIFICATIONS:
!     ------------------------------------------------------------------
...
!     ARGUMENTS
!     ------------------------------------------------------------------
      INTEGER, INTENT(IN):: Itest, Igrid
      INTEGER:: DIM1
      !INTEGER ij, ic, ir, il, i, i1, i2
!     -----------------------------------------------------------------
      INTERFACE
