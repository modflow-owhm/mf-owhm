...
47    FORMAT(1X, ' POND DIVERSION RATES WILL BE READ FROM TIME ',
     ...
     + ' ROW ENTRIES WILL BE READ FROM EACH FILE')
      ...
      CHARACTER*6 CWELL
      CHARACTER(20),DIMENSION(1):: AGGROUP
      ! - -----------------------------------------------------------------
      AGGROUP = 'NOGROUP'  ! To complete call to ULSTRD, NOGROUP indicates group reading is not supported
      ! - -----------------------------------------------------------------
      found4 = .false.
      found7 = .false.
      ierror = .false.
...
!
      ...
      ! - -----------------------------------------------------------------
      USE GWFAGMODULE
      USE GWFSFRMODULE, ONLY: SEG, NUMTAB, ISFRLIST, SEGINFLOWSAVE
...
11    FORMAT(' A total number of ', i10, ' AG output time series files '
     ...
     +                  RHS, IUNIT
      USE GWFBASMODULE, ONLY: TOTIM
      USE GWFAGMODULE
      USE GWFSFRMODULE, ONLY: SEG, DVRSFLW, SGOTFLW, NUMTAB
...
!1 - ------RESET DEMAND IF IT CHANGES for MODSIM simulations
      ...
      !         ! has not yet overwritten values in SEG(2,x), SEG(2,x) still 
      !         ! contains the TABFILE values at this point.
      !      IF ( NUMTAB > 0 )  DEMAND(ISEG) = SEG(2, ISEG)
      !      SUPACT(ISEG) = 0.0
      !      ACTUAL(ISEG) = 0.0
      !    end if
      !  END DO
