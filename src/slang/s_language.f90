    ...
    DO WHILE (BL%LIST%IS_ASSOCIATED())
                    ...
                    IF(WORD .NE. 'CONSTANT') THEN
                       ...
                       IF(.NOT. FL%IS_CONSTANT) THEN
                          ...
                          IF(FL%IS_INTERNAL .AND. N==Z) CALL STOP_ERROR( LINE=BL%LINE, INFILE=BL%IU, OUTPUT=BL%IOUT, MSG= 'LOOKUP TABLE LOAD BLOCK ERROR: INTERNAL KEYWORD IS NOT SUPPORTED WITH AUTO-COUNT (SETTING COUNT<1).'//NL//'PLEASE MOVE THE TIME SERIES TO A SEPARATE FILE OR SPECIFY THE NUMBER OF LINES IT CONTAINS.')
