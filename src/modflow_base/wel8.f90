...
!  ------------------------------------------------------------------
  USE GWFWEL2MODULE
  USE GLOBAL,                           ONLY: LIST_UNIT=>IOUT,IFREFM,IUNIT
  ...
  !
  INTEGER:: I, J, K, LLOC, ISTART, ISTOP, IUNITNWT, NINLST
...
! ------------------------------------------------------------------
  ...
  ALLOCATE(IOUT)
  IOUT = LIST_UNIT
  CALL READ_TO_DATA(LINE,IN,IOUT,IOUT, HED="-- READING WEL PACKAGE INPUT --")
  !
  ! SETUP PACKAGE UNITS:
  IUNITNWT=IUNIT(63)
  !
...
!1------IDENTIFY PACKAGE AND INITIALIZE NWELLS.
  WRITE(IOUT,'(/A/)') 'WEL -- WELL PACKAGE, VERSION OWHM INPUT READ FROM UNIT '//NUM2STR(IN)
  NWELLS=Z
  NNPWEL=Z
  !
  LLOC = 1
  ...
  IF(LINE(ISTART:ISTOP).EQ.'SPECIFY') THEN
      !
      WELSMOOTHING = TRUE
      CALL GET_NUMBER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,PHIRAMP,MSG='FOUND KEYWORD "SPECIFY" WHICH MUST FOLLOW WITH PHIRAMP VALUE, BUT FAILED TO CONVERT IT TO A FLOATING POINT NUMBER.')
      !
      ...
      IF(ISTART >= ISTART) THEN
          !
          CALL IUNITRAMP%OPEN(LINE, LLOC, IOUT, IN, NO_BINARY=TRUE, SAVE_FNAME=TRUE, ALLOW_ONLY_UNIT=TRUE)
          !
...
!3------READ AUXILIARY VARIABLES AND PRINT FLAG.
  ...
  DO !BLOCK GROUPS
     ...
     ELSEIF(BL%NAME == 'LINEFEED') THEN
         ...
         IF(BL%NLINE>0) THEN
             CALL WELFEED%INIT(BL, LABEL="Q")    !=>FEED_ALLOCATE(IN,IOUT,LINE)
             NO_LINEFEED = FALSE
         END IF
     !
     ELSEIF((BL%NAME == 'OPTION' .OR. BL%NAME == 'OPTIONS') .AND. BL%NLINE>Z) THEN
         ...
         DO I=1, BL%NLINE
             ...
             CASE('SMOOTHING')
                             ...
                             END IF
                             CALL GET_NUMBER(BL%LINE,LLOC,ISTART,ISTOP,IOUT,IN,PHIRAMP,MSG='FOUND OPTION "SMOOTHING" WHICH MUST FOLLOW WITH PHIRAMP VALUE, BUT FAILED TO CONVERT IT TO A FLOATING POINT NUMBER.')
                             !
                             IF(SGN > DZ) THEN
                             ...
                             IF(ISTART >= ISTART) THEN
                                 IF(BL%LINE(ISTART:ISTOP) .NE. BLNK) CALL IUNITRAMP%OPEN(BL%LINE, LLOC, IOUT, IN, NOSTOP=TRUE, NO_BINARY=TRUE, SAVE_FNAME=TRUE, ALLOW_ONLY_UNIT=TRUE)
                             END IF
             CASE('AUXILIARY', 'AUX')
                             CALL PARSE_WORD_UP(BL%LINE,LLOC,ISTART,ISTOP,TRUE)
                             IF(NAUX.LT.20) THEN
  ...
  !
  !ALLOCATE WELFEED VARIABLE AND OPTIONALLY READ IN FEED FILE LOCATIONS
  IF(NO_LINEFEED) CALL WELFEED%INIT(IN,IOUT,LINE,LABEL="Q")    ! =>FEED_ALLOCATE(IN,IOUT,LINE)
  !
  LLOC = ONE
  IF(IFREFM.EQ.Z) THEN
     !
     ...
     !
     IF(MXACTW == inf_I .AND. NO_LINEFEED) CALL STOP_ERROR(LINE,IN,IOUT, MSG=NL//'ERROR: WEL PACKAGE DOES NOT HAVE ANY LINEFEED FILES SPECIFIED, SO YOU MUST READ "MXACTW", BUT GET_INTEGER FAILED TO LOAD THAT NUMBER.')
     IF(MXACTW == inf_I) MXACTW = Z
     !
     LLOC = ONE
     IF(NO_CBC_READ) CALL GET_INTEGER(LINE(11:20),LLOC,ISTART,ISTOP,IOUT,IN,IWELCB,MSG='NOSTOP')
     IF(IWELCB == inf_I)  CALL STOP_ERROR(LINE,IN,IOUT, MSG=NL//'WEL PACKAGE OPTIONS ERROR: DID NOT FIND WITHIN THE OPTIONS BLOCK THE KEYWORD "WELL_CBC", SO INPUT EXPECTS TO LOAD AFTER MXACTW THE CBC UNIT NUMBER "IWELCB".')
  ELSE
     !
  ...
  CALL CHECK_CBC_GLOBAL_UNIT(IWELCB, .FALSE.)
  !
...
!        SPECIFICATIONS:
!     ------------------------------------------------------------------
  USE GLOBAL,        ONLY:IFREFM
  USE GWFWEL2MODULE, ONLY:NWELLS,MXWELL,NWELVL,IPRWEL,NPWEL,IWELPB,  &
                          NNPWEL,WELAUX,WELDATA,WELTABFILE,WELFEED, EOF_WARNING_GIVEN, WELBUD, IOUT
...
!1----AND NUMBER OF PARAMETERS
  ...
  ELSE
   IF(.NOT. EOF_WARNING_GIVEN) THEN
       WRITE(IOUT,'(/A)')"WEL PACKAGE WARNING (don't panic): FAILED TO READ ITMP (STRESS PERIOD INFORMATION)."//NEW_LINE(' ')//  &
      'THIS MAY BE DUE TO THE END OF THE FILE BEING REACHED IN THE WELL PACKAGE INPUT FILE.'//NEW_LINE(' ')//                    &
      'ITMP IS SET TO ZERO FOR THE REST OF THE SIMULATION.'//NEW_LINE(' ')//                                                     &
...
!     ------------------------------------------------------------------
  ...
  INTEGER, INTENT(IN):: IGRID
  !
  DOUBLE PRECISION Qp,Hh,Ttop,Bbot
  REAL:: Q, ZERO
...
! ------------------------------------------------------------------
  ...
  USE GWFWEL2MODULE,ONLY:IWELCB,WELDATA,NWELVL,WELAUX,PHIRAMP,IUNITRAMP,IPRWEL,WELSMOOTHING,WELBUD,IOUT
  !
