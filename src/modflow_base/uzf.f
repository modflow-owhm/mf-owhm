...
C     ARGUMENTS
C     ------------------------------------------------------------------
      INTEGER In, Iunitbcf, Iunitlpf, Iunithuf, Iunitupw, Iunitlmt,Igrid
C     ------------------------------------------------------------------
C     LOCAL VARIABLES
C     ------------------------------------------------------------------
...
C1------IDENTIFY PACKAGE AND INITIALIZE.
 ...
 9001 FORMAT (1X, /' UZF1 -- UNSATURATED FLOW PACKAGE, VERSION OWHM', 
     +            /, 9X, 'INPUT READ FROM UNIT', I5)
!
      IOUT = LIST_UNIT
      CALL READ_TO_DATA(LINE,IN,IOUT,IOUT,
     +                          HED="-- READING UZF PACKAGE INPUT --")
      !
...
C SWR - JDH ADDED Iunitswr
     ...
     +                    Iunitcfp, Iunitnwt, Iunitswr,Iunitagy, Igrid)
C     ******************************************************************
...
C     -----------------------------------------------------------------
      ...
      INTEGER Iunitnwt, Iunitswr, Iunitagy
C     -----------------------------------------------------------------
C     LOCAL VARIABLES
C     -----------------------------------------------------------------
...
C------SUBROUTINE GWF2UZF1BD
      SUBROUTINE GWF2UZF1BD(Kkstp, Kkper, Iunitlak, Iunitsfr, 
     +                      Iunitagy, Igrid)
C     ******************************************************************
...
C     ARGUMENTS
C     -----------------------------------------------------------------
      INTEGER Kkper, Kkstp, Igrid, Iunitlak, Iunitsfr, Iunitagy
C     -----------------------------------------------------------------
C     LOCAL VARIABLES
C     -----------------------------------------------------------------
      TYPE(WARNING_TYPE):: WRN
...
C4------CLEAR BUFFERS.
CDEP 05/05/2006
      CALL SET_ARRAY(NCOL, NROW, NLAY, HNOFLO, BUFF)
      !CALL INITARRAY(TOTCELLS, HNOFLO, BUFF(:,:,1)) !seb this seems wrong: TOTCELLS=NROW*NCOL*NLAY, should this be NUMCELLS=NROW*NCOL
      DO iuzrat = 1, 8
...
! Suppress SEEPOUT beneath a lake
            IF ( lakflg.NE.1 .AND. Iseepsupress.EQ.0 ) THEN
              ...
              csep = (csepmx/SURFDEP)*(h-celtop)
              IF ( csep .GT. csepmx ) csep = csepmx
              IF ( csep .LT. zero ) csep = zero
              SEEPOUT(ic, ir) = (h-celtop)*csep
              IF( SEEPOUT(ic, ir).LT.zero ) SEEPOUT(ic, ir) = zero 
            END IF
            IF ( SURFDEP.GT.CLOSEZERO ) THEN
              finfact = (finfhold/SURFDEP)*((celtop+SURFDEP)-h)
            END IF
            IF ( finfact.LT.zero ) finfact = zero
            IF ( finfact.GT.finfhold ) finfact = finfhold
            IF ( finfact.GT.CLOSEZERO ) THEN 
              REJ_INF(ic, ir) = cellarea * (finfhold - finfact)
            ELSE
            ...
            IF ( IUZFOPT.GT.0 ) THEN
              IF ( ibnd.GT.0 ) THEN
                IF ( iss.EQ.0 ) THEN
                  ...
                  dlength = 0.0D0
                  zoldist = UZDPST(iset, l)
                  IF ( dlength.LT.zero ) dlength = 0.0D0
                  IF ( zoldist.LT.zero ) zoldist = 0.0D0
                  oldsflx = UZOLSFLX(ic, ir)
                  surflux = finfact
                  numwaves = NWAVST(ic, ir)
                  DO ik = 1, idelt
...
C         ABOVE WATER TABLE.                            
     ...
     +                                  /(ths-thr)
                                IF ( fhold.LT.CLOSEZERO ) fhold = zero
...
C Print net recharge as ascii to a separate output file
      IF ( INETFLUX.GT.0 ) THEN
        TIMEPRINT = TIMEPRINT + DELT  
           ...
           !CALL INITARRAY(TOTCELLS,0.0,BUFF(:,:,1)) !seb this seems wrong: TOTCELLS=NROW*NCOL*NLAY, should this be NUMCELLS=NROW*NCOL
           DO ir = 1, NROW
             DO ic = 1, NCOL
               IF ( IUZFBND(ic,ir).NE.0 ) THEN
                  dum1 = zero
                  ...
                  IF ( Iunitsfr.GT.0 ) THEN
                    IF ( FNETSEEP(ic,ir).GT.zero ) THEN
...
C-----SAVE NET DISCHARGE RATES TO UNFORMATTED FILE FOR UZF OR MODFLOW BUDGET ITEMS.                
     ...
     +                  PERTIM, TOTIM, IBOUND)
            DO ir = 1, NROW
              DO ic = 1, NCOL
                FNETEXFIL1(ic, ir) = zero
                FNETEXFIL2(ic, ir) = zero    
              END DO
            END DO
          END IF
        END IF
...
C1------DETERMINE IF WATER TABLE IS RISING OR FALLING.
     ...
     &       'UZFLOW2 - NUMBER OF WAVES GRATER THAN NSETS2.'//BLN//
     &       'THE NUMBER OF WAVES FOR CELL (ROW, COL) = ('//
     &        NUM2STR(IR)//', '//NUM2STR(IC)//') IS '//
     &        NUM2STR(Numwaves)//BLN//'PLEASE  INCREASE NSETS2')
          END IF
        ELSE
          Depth(Jpnt) = Dlength
        END IF
...
C1------INITIALIZE TRAILING WAVES.
      ...
      IF ( Theta(jpnwavesm2)-smoist.GT.feps3 ) THEN
        ...
        DO j = Numwaves, Numwaves + numtrail2 - 1
          IF ( j.GT.NWAV ) THEN
            CALL STOP_ERROR(OUTPUT=IOUT, MSG=
     &     'UZF TRAILWAVE2 - NUMBER OF WAVES GRATER THAN NSETS2.'//BLN//
     &     'THE NUMBER OF WAVES IS '//NUM2STR(Numwaves)//BLN//
     &     'PLEASE INCREASE NSETS2')
          END IF
          jpntpjm1 = jpntm1 + j
          Ltrail(jpntpjm1) = 0
          Itrwave(jpntpjm1) = 0
...
C     ******************************************************************
     ...
     +                        ZEROD9, ZEROD7, ETOFH_FLAG, ROOTACT,IOUT
...
!  rgn added new calculation for speed 5/26/09.
     ...
     +                              Flux(Jpnt+Numwaves))/bottom
            ...
            CALL STOP_ERROR(OUTPUT=IOUT, MSG=
     &  'UZF TRANSPIRATION - NUMBER OF WAVES GRATER THAN NSETS2.'//BLN//
     &  'THE NUMBER OF WAVES FOR CELL (ROW, COL) = ('//
     &   NUM2STR(IR)//', '//NUM2STR(IC)//') IS '//
     &   NUM2STR(Numwaves)//BLN//'PLEASE  INCREASE NSETS2')
          END IF
...
!
      !PURE SUBROUTINE INITARRAY(N,Val,A)
      !INTEGER,            INTENT(IN) :: N
      !REAL,               INTENT(IN) :: Val
...
!      
      PURE FUNCTION Rate_ET_Z(FACTOR,FKTHO,HROOT,H,ROOT) RESULT(ETZ)
        USE CONSTANTS, ONLY: DZ
        DOUBLE PRECISION, INTENT(IN):: FKTHO,HROOT,H,FACTOR,ROOT
        DOUBLE PRECISION:: ETZ
        !
        ETZ = ROOT*FACTOR*FKTHO*(H-HROOT)
        IF ( ETZ < DZ ) ETZ = DZ
      END FUNCTION
!
      PURE FUNCTION CAPH(THTR,THO,THSAT,HA,EPS)
        USE CONSTANTS, ONLY: DNEG, DZ, DOS, TRES, NEARZERO_15
        DOUBLE PRECISION, INTENT(IN):: THTR,THO,HA
        REAL,             INTENT(IN):: THSAT,EPS
        DOUBLE PRECISION:: CAPH
        DOUBLE PRECISION:: STAR,LAMBDA
        !
        STAR   = (THO-THTR)/DBLE(THSAT-THTR) 
        LAMBDA = DOS/(EPS - TRES)    
        IF (STAR > NEARZERO_15 .AND. THO-DBLE(THSAT) < NEARZERO_15) THEN
                                        CAPH = HA*STAR**(DNEG/LAMBDA)
        ELSE
                                        CAPH   = DZ
        END IF 
      END FUNCTION
