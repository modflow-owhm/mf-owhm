...
c     ------------------------------------------------------------------
     ...
     +           Iunwt, Igrid
      CHARACTER(*) :: Fname                 !!08/19/02KJH-MODIFIED
c     ------------------------------------------------------------------
c     Local Variables
c     ------------------------------------------------------------------
      REAL :: bs
...
c     ------------------------------------------------------------------
     ...
     +          HREF(NCOL,NROW,NLAY))
      ...
      CALL READ_TO_DATA(txt,IN,IOUT,IOUT,BACK_UP=.TRUE.,
     +                          HED="-- READING MNW1 PACKAGE INPUT --")
c
      IOWELL2(1) = 0
      IOWELL2(2) = 0
      IOWELL2(3) = 0
...
c1------identify package and initialize nwell2
 ...
 9001 FORMAT (/, ' MNW1 -- MULTI-NODE WELL 1 PACKAGE, VERSION OWHM', 
     +            /, '    INPUT READ FROM UNIT', A)
      NWELL2 = 0
c
...
c
      ...
      CALL CHECK_CBC_GLOBAL_UNIT(IWL2CB, .FALSE.)
      !
c
...
c
      ...
      IF ( kf.GT.0 ) THEN
        ...
        tx2 = MNWNAME
        CALL UPCASE(tx2)
        IF ( INDEX(tx2, 'FILEPREFIX').GT.0 ) THEN  !seb This could be a problem if FNAME has a path ./path/Name.nam
          MNWNAME = Fname
          ke = INDEX(MNWNAME, '.')
          MNWNAME(ke:200) = '               '
        ENDIF
...
!      IF ( Txt(1:1).EQ.'#' ) GOTO 5
    5 CALL READ_TO_DATA(Txt,Io,EOF=EOF)
      IF(EOF) GOTO 10
c
      ki = INDEX(Txt, '!')
      IF ( ki.GT.0 )
