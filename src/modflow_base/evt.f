...
C2------IDENTIFY PACKAGE.
      IEVTPF=0
        WRITE(IOUT,1)IN
    1 FORMAT(1X,/1X,'EVT -- EVAPOTRANSPIRATION PACKAGE, VERSION OWHM,',
...
C3------TERMS (IEVTCB).
     ...
     +                          HED="-- READING EVT PACKAGE INPUT --")
      ...
      CALL CHECK_CBC_GLOBAL_UNIT(IEVTCB, .FALSE.)
      !
C
...
C8------READ NAMED PARAMETERS
    ...
    7       FORMAT(1X,'Parameter type must be EVT')
            CALL USTOP('Parameter type must be EVT')
         END IF
   20    CONTINUE
      END IF
C
...
C
      ...
      NOFREE = IFREFM == 0
C
      ANAME(1) = '          ET LAYER INDEX'
      ANAME(2) = '              ET SURFACE'
      ANAME(3) = ' EVAPOTRANSPIRATION RATE'
      ANAME(4) = '        EXTINCTION DEPTH'
C     ------------------------------------------------------------------
C
