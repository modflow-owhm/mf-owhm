...
C     SPECIFICATIONS:
     ...
     2                      NSTP,LBOTM,BOTM
        ...
        USE GWFSWIMODULE
        USE CONSTANTS, ONLY: NEG, Z, ONE, DZ, UNO
        IMPLICIT NONE
C       + + + DUMMY ARGUMENTS + + +
...
C       + + + DATA + + +
        csolver(1) = '                     DIRECT SOLVER (DE4)'
        csolver(2) = ' PRECONDITIONED CONJUGATE GRADIENT (PCG)'
...
C     + + + INPUT FORMATS + + +
C     + + + OUTPUT FORMATS + + +
    1   FORMAT(//1X,'SWI2 -- SWI PACKAGE, VERSION OWHM',
     2          /1X,50('-'),
        ...
        DO
          ...
          SELECT CASE ( line(istart:istop) )
            ...
            CASE ( '0', '', '#' )
              EXIT
            CASE DEFAULT
!              IF ( LEN(line(istart:istop)) == Z ) EXIT
...
C             THE SPECIFIED ZETA VALUE IS CLOSER TO THE TOP OR BOTTOM
            ...
            I_ZETA: DO i = 1, NROW
              J_ZETA: DO j = 1, NCOL
                ...
                zt = ZETA(j,i,k,iz)
                IF (zt.NE.bbot) THEN
                  IF (zt.LT.(bbot+d)) THEN
                    ZETA(j,i,k,iz) = bbot
                  END IF
                END IF
                IF (zt.GT.(ttop-d)) ZETA(j,i,k,iz) = ttop
              END DO J_ZETA
            END DO I_ZETA
          END DO K_ZETA
        END DO IZ_ZETA
...
C           INITIALIZE FLOAT ARRAYS
          DO k = 1, NLAYSWI
            DO i = 1, NROW
              DO j = 1, NCOL
                SWIPCG%VPCG(j,i,k) = DZ
                SWIPCG%SS(j,i,k)   = DZ
                SWIPCG%P(j,i,k)    = DZ
                SWIPCG%HPCG(j,i,k) = DZ
                SWIPCG%CD(j,i,k)   = DZ
                IF ( SWIPCG%NPCOND.EQ.2 ) THEN
                  SWIPCG%HCSV(j,i,k) = DZ
                END IF
              END DO
            END DO
          END DO
...
C
        ...
        textch  = '      SWIADDTOCH'
        bdtext(1)  = '      BOUNDARIES'
        bdtext(2)  = '   CONSTANT HEAD'
        bdtext(3)  = '     ZONE CHANGE'
        bdtext(4)  = 'ZONE CHG TIP/TOE'
        bdtext(5)  = '     ZONE MIXING'
C
...
C       + + + DUMMY ARGUMENTS + + +
        ...
        INTEGER:: J
        !
        ! Deallocate the GWFSWIDAT Module Variables -------------------------------------------
        !
        DEALLOCATE( GWFSWIDAT(Igrid)% IOUT       , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NSRF       , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ISTRAT     , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NSWIOPT    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NZONES     , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% IFIXED     , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NADPTFLG   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NADPTMX    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NADPTMN    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ADPTFCT    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% IADPT      , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% IADPTMOD   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ADPTVAL    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% SWIDELT    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NADPTSUM   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% RADPTSUM   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% IADPTSUM   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NOBS       , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% IOBSHEADER , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ISWIZT     , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ISWICB     , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ISWIOBS    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NLAYSWI    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NSOLVER    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% IPRSOL     , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% MUTSOL     , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% TOESLOPE   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% TIPSLOPE   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ALPHA      , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% BETA       , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ICONV      , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% IBO        , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% SWIHCOF    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% SWISOLCR   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% SWISOLCC   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% SWISOLCV   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ZETA       , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ZETAOLD    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ZETASWITS0 , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ZONECHG1   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ZONECHG2   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% ZONEIMIX   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% SSZ        , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% EPS        , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NUS        , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% DELNUS     , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NUSRF      , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% SWICR      , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% SWICC      , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% SWICUMCR   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% SWICUMCC   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NUTOP      , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NUBOT      , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% QLEXTRA    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% QREXTRA    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% QFEXTRA    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% QLEXTRACUM , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% QREXTRACUM , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% QFEXTRACUM , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% BRHS       , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% DUM        , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% RHSPRESWI  , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% IPLPOS     , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% IZONENR    , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% NBDITEMS   , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% CUMBD      , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% INCBD      , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% RRATIN     , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% RRATOUT    , STAT=J)
        !DEALLOCATE( GWFSWIDAT(Igrid)% SC1        , STAT=J) --> These were never allocated, but instead pointed to another modules variable
        !DEALLOCATE( GWFSWIDAT(Igrid)% SC2        , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% SWIOBS     , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% SWIDE4     , STAT=J)
        DEALLOCATE( GWFSWIDAT(Igrid)% SWIPCG     , STAT=J)  
        !
        ! Nullify LGR Data Types  -------------------------------------------
        ! 
        NULLIFY( GWFSWIDAT(Igrid)% IOUT       )
        NULLIFY( GWFSWIDAT(Igrid)% NSRF       )
        NULLIFY( GWFSWIDAT(Igrid)% ISTRAT     )
        NULLIFY( GWFSWIDAT(Igrid)% NSWIOPT    )
        NULLIFY( GWFSWIDAT(Igrid)% NZONES     )
        NULLIFY( GWFSWIDAT(Igrid)% IFIXED     )
        NULLIFY( GWFSWIDAT(Igrid)% NADPTFLG   )
        NULLIFY( GWFSWIDAT(Igrid)% NADPTMX    )
        NULLIFY( GWFSWIDAT(Igrid)% NADPTMN    )
        NULLIFY( GWFSWIDAT(Igrid)% ADPTFCT    )
        NULLIFY( GWFSWIDAT(Igrid)% IADPT      )
        NULLIFY( GWFSWIDAT(Igrid)% IADPTMOD   )
        NULLIFY( GWFSWIDAT(Igrid)% ADPTVAL    )
        NULLIFY( GWFSWIDAT(Igrid)% SWIDELT    )
        NULLIFY( GWFSWIDAT(Igrid)% NADPTSUM   )
        NULLIFY( GWFSWIDAT(Igrid)% RADPTSUM   )
        NULLIFY( GWFSWIDAT(Igrid)% IADPTSUM   )
        NULLIFY( GWFSWIDAT(Igrid)% NOBS       )
        NULLIFY( GWFSWIDAT(Igrid)% IOBSHEADER )
        NULLIFY( GWFSWIDAT(Igrid)% ISWIZT     )
        NULLIFY( GWFSWIDAT(Igrid)% ISWICB     )
        NULLIFY( GWFSWIDAT(Igrid)% ISWIOBS    )
        NULLIFY( GWFSWIDAT(Igrid)% NLAYSWI    )
        NULLIFY( GWFSWIDAT(Igrid)% NSOLVER    )
        NULLIFY( GWFSWIDAT(Igrid)% IPRSOL     )
        NULLIFY( GWFSWIDAT(Igrid)% MUTSOL     )
        NULLIFY( GWFSWIDAT(Igrid)% TOESLOPE   )
        NULLIFY( GWFSWIDAT(Igrid)% TIPSLOPE   )
        NULLIFY( GWFSWIDAT(Igrid)% ALPHA      )
        NULLIFY( GWFSWIDAT(Igrid)% BETA       )
        NULLIFY( GWFSWIDAT(Igrid)% ICONV      )
        NULLIFY( GWFSWIDAT(Igrid)% IBO        )
        NULLIFY( GWFSWIDAT(Igrid)% SWIHCOF    )
        NULLIFY( GWFSWIDAT(Igrid)% SWISOLCR   )
        NULLIFY( GWFSWIDAT(Igrid)% SWISOLCC   )
        NULLIFY( GWFSWIDAT(Igrid)% SWISOLCV   )
        NULLIFY( GWFSWIDAT(Igrid)% ZETA       )
        NULLIFY( GWFSWIDAT(Igrid)% ZETAOLD    )
        NULLIFY( GWFSWIDAT(Igrid)% ZETASWITS0 )
        NULLIFY( GWFSWIDAT(Igrid)% ZONECHG1   )
        NULLIFY( GWFSWIDAT(Igrid)% ZONECHG2   )
        NULLIFY( GWFSWIDAT(Igrid)% ZONEIMIX   )
        NULLIFY( GWFSWIDAT(Igrid)% SSZ        )
        NULLIFY( GWFSWIDAT(Igrid)% EPS        )
        NULLIFY( GWFSWIDAT(Igrid)% NUS        )
        NULLIFY( GWFSWIDAT(Igrid)% DELNUS     )
        NULLIFY( GWFSWIDAT(Igrid)% NUSRF      )
        NULLIFY( GWFSWIDAT(Igrid)% SWICR      )
        NULLIFY( GWFSWIDAT(Igrid)% SWICC      )
        NULLIFY( GWFSWIDAT(Igrid)% SWICUMCR   )
        NULLIFY( GWFSWIDAT(Igrid)% SWICUMCC   )
        NULLIFY( GWFSWIDAT(Igrid)% NUTOP      )
        NULLIFY( GWFSWIDAT(Igrid)% NUBOT      )
        NULLIFY( GWFSWIDAT(Igrid)% QLEXTRA    )
        NULLIFY( GWFSWIDAT(Igrid)% QREXTRA    )
        NULLIFY( GWFSWIDAT(Igrid)% QFEXTRA    )
        NULLIFY( GWFSWIDAT(Igrid)% QLEXTRACUM )
        NULLIFY( GWFSWIDAT(Igrid)% QREXTRACUM )
        NULLIFY( GWFSWIDAT(Igrid)% QFEXTRACUM )
        NULLIFY( GWFSWIDAT(Igrid)% BRHS       )
        NULLIFY( GWFSWIDAT(Igrid)% DUM        )
        NULLIFY( GWFSWIDAT(Igrid)% RHSPRESWI  )
        NULLIFY( GWFSWIDAT(Igrid)% IPLPOS     )
        NULLIFY( GWFSWIDAT(Igrid)% IZONENR    )
        NULLIFY( GWFSWIDAT(Igrid)% NBDITEMS   )
        NULLIFY( GWFSWIDAT(Igrid)% CUMBD      )
        NULLIFY( GWFSWIDAT(Igrid)% INCBD      )
        NULLIFY( GWFSWIDAT(Igrid)% RRATIN     )
        NULLIFY( GWFSWIDAT(Igrid)% RRATOUT    )
        NULLIFY( GWFSWIDAT(Igrid)% SC1        )
        NULLIFY( GWFSWIDAT(Igrid)% SC2        )
        NULLIFY( GWFSWIDAT(Igrid)% SWIOBS     )
        NULLIFY( GWFSWIDAT(Igrid)% SWIDE4     )
        NULLIFY( GWFSWIDAT(Igrid)% SWIPCG     ) 
        !
