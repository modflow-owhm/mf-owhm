...
C     TABGROUPNAME EXTERNAL FILENUMBER
      DO I=ONE,TABFILE%NTAB
        ...
        EXT=LINE(ISTART:ISTOP)                            !Check for file keyword
        CALL UPCASE(EXT)
        IF (EXT == 'OPEN/CLOSE' .OR. EXT == 'DATAFILE')THEN
               CALL URWORD(LINE,LLOC,ISTART,ISTOP,Z,N,R,IOUT,IN) ! Move to actual file
        ELSEIF(EXT == "SKIP"      .OR. 
     +         EXT == "CONSTANT"  .OR. 
     +         EXT == "INTERNAL"  .OR. 
     +         EXT == "REPEAT"    .OR. 
     +         EXT == "RELOAD"    .OR. 
     +         EXT == "LOAD_NEXT"     ) THEN
         CALL FILE_IO_ERROR(LINE=LINE,INFILE=IN,OUTPUT=IOUT,
     +       MSG='"'//EXT//
     +           '" KEYWORD IS NOT SUPPORTED FOR OPENING A TABFILE')
        END IF
        !
        FNAME=' '
        IF (EXT == 'EXTERNAL' .OR. EXT == 'DATAUNIT')THEN
     ...
     +       MSG='FAILED TO OPEN TABFILE. PLEASE CHECK INPUT LINE')
           END IF
           !
...
C LOCAL
      ...
      END TYPE
      TYPE (LINKEDLIST),POINTER:: DATASTR
      TYPE (LINKEDLIST),POINTER:: DATAPNT
      LOGICAL:: ALLOC_DATASTR
      !
      IF(TABFILE%USEVAL) RETURN                                         !FLAG THAT INDICATES THAT THE TABFILE HAS ALREADY BEEN INTERPOLATED TO CURRENT TIME
      !
