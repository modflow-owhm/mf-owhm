      MODULE GWFLPFMODULE
        ...
        INTEGER, SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS:: LAYTYP
        INTEGER, SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS:: LAYAVG
        REAL,    SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS:: CHANI
        INTEGER, SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS:: LAYVKA
        ...
        REAL,    SAVE,   POINTER, DIMENSION(:,:,:),CONTIGUOUS:: VKA
      ...
      TYPE GWFLPFTYPE
        ...
        REAL, POINTER   :: WETFCT
        INTEGER,   POINTER, DIMENSION(:)    ,CONTIGUOUS:: LAYTYP
        INTEGER,   POINTER, DIMENSION(:)    ,CONTIGUOUS:: LAYAVG
        REAL,      POINTER, DIMENSION(:)    ,CONTIGUOUS:: CHANI
        INTEGER,   POINTER, DIMENSION(:)    ,CONTIGUOUS:: LAYVKA
        ...
        REAL,      POINTER, DIMENSION(:,:,:),CONTIGUOUS:: VKA
...
C2------IDENTIFY PACKAGE
        WRITE(IOUT,1) IN
    1 FORMAT(1X,/1X,'LPF -- LAYER-PROPERTY FLOW PACKAGE, VERSION OWHM',
     1             /,9X,'INPUT READ FROM UNIT ',I4)
C
C3------READ COMMENTS AND ITEM 1.
      CALL READ_TO_DATA(LINE,IN,IOUT,IOUT,
     +                  HED="-- READING LPF PACKAGE INPUT --")
      LLOC=1
      ...
      CALL CHECK_CBC_GLOBAL_UNIT(ILPFCB, .TRUE.)
      !
C
C3A-----WRITE ITEM 1
        IF(ILPFCB < Z) WRITE(IOUT,8)
    8 FORMAT(1X,'CONSTANT-HEAD CELL-BY-CELL FLOWS WILL BE PRINTED',
     1  ' WHEN ICBCFL IS NOT 0')
        IF(ILPFCB > Z) WRITE(IOUT,9) ILPFCB
...
C3B-----GET OPTIONS.
   ...
   30    FORMAT(1X,'NOPARCHECK  OPTION:',/,1X,
     ...
     2        'parameters define data at all cells')
      ELSE IF(LINE(ISTART:ISTOP).EQ.'LPF_STORAGE' .OR. 
     +        LINE(ISTART:ISTOP).EQ.'LPF'             ) THEN         ! Option not used in LPF, but included to have same input as UPW
           IF(LINE(ISTART:ISTOP).EQ.'LPF') 
     +                       CALL PARSE_WORD(LINE,LLOC,ISTART,ISTOP)
     +      
      END IF
      IF(LLOC < 650) GO TO 20
C
...
C4B-----TO SC2, HANI, and WETDRY.  PRINT INTERPRETED VALUES OF FLAGS.
   ...
   67 FORMAT(1X,/3X,'INTERPRETATION OF LAYER FLAGS:',/1X,
     ...
     +      NUM2STR(K)//NL//'LAYWET must be 0 if LAYTYP is 0')
         ...
         END IF
      END IF
      IF(LAYAVG(K) < Z .OR. LAYAVG(K).GT.2) THEN
   ...
   74    FORMAT(1X,I8,
     1    ' IS AN INVALID LAYAVG VALUE -- MUST BE 0, 1, or 2')
      ...
      END IF
      LAYPRN(1)=TYPNAM(1)
      IF(LAYTYP(K) /= Z) LAYPRN(1)=TYPNAM(2)
