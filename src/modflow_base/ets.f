...
C     ------------------------------------------------------------------
  500 FORMAT(1X,/
     &1X,'ETS7 -- EVAPOTRANSPIRATION SEGMENTS PACKAGE, VERSION OWHM,',
     &            /,9X,'INPUT READ FROM UNIT ',I4)
  510 FORMAT(
...
C       SEGMENTS (NETSEG) (ITEM 1)
      ...
      CALL CHECK_CBC_GLOBAL_UNIT(IETSCB, .FALSE.)
      !
C
...
C3A-----OPTION IS ILLEGAL -- PRINT A MESSAGE & ABORT SIMULATION.
        WRITE(IOUT,540)
      CALL USTOP('ILLEGAL ET OPTION CODE')
C
...
C     PRINT MESSAGE IDENTIFYING NUMBER OF SEGMENTS IN ET VS. HEAD CURVE
      ...
      ELSE
          WRITE(IOUT,530)
        CALL USTOP('EVAPOTRANSPIRATION RATE FUNCTION MUST CONTAIN AT'//
     &             ' LEAST ONE SEGMENT')
      ENDIF
C
...
C         UPARARRRP READS PARAMETER NAME AND DEFINITION (ITEMS 2 AND 3)
   ...
   57       FORMAT(1X,'Parameter type must be ETS')
            CALL USTOP('Parameter type must be ETS')
          ENDIF
  100   CONTINUE
      ENDIF
C
