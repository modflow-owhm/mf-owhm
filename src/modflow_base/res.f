...
C
      ...
      LOGICAL:: NOFREE
      NOFREE = IFREFM == 0
      ANAME(1) = '      RESERVOIR LOCATION'
      ANAME(2) = '   RESERVOIR LAYER INDEX'
      ANAME(3) = 'RESERVOIR LAND SURF ELEV'
      ANAME(4) = '  RES. BED VERT HYD COND'
      ANAME(5) = ' RESERVOIR BED THICKNESS'
C     ------------------------------------------------------------------
      ALLOCATE (NRES,IRESCB,NRESOP,IRESPT,NPTS)
C
C1------IDENTIFY PACKAGE AND INITIALIZE
        WRITE(IOUT,1)IN
    1 FORMAT(/,'RES7 -- RESERVOIR PACKAGE, VERSION OWHM',
...
C2------RESERVOIR OPTIONS
     ...
     +                  HED="-- READING RES PACKAGE INPUT --")
      IF(NOFREE) THEN
          READ(LINE,"(5I10)") NRES,IRESCB,NRESOP,IRESPT,NPTS
      ELSE
          LLOC=1
          CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,NRES,
     +                            MSG='MNWI FAILED TO READ NRES')
          CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,IRESCB,
     +                            MSG='MNWI FAILED TO READ IRESCB')
          CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,NRESOP,
     +                            MSG='MNWI FAILED TO READ NRESOP')
          CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,IRESPT,
     +                            MSG='MNWI FAILED TO READ IRESPT')
          CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,NPTS,
     +                            MSG='MNWI FAILED TO READ NPTS')
      END IF
      !
      ! CHECK IF GLOBAL SHUTDOWN OF CBC IS IN EFFECT
       CALL CHECK_CBC_GLOBAL_UNIT(IRESCB, .FALSE.)
      !
C
...
C2A-----PRINT VALUE
    ...
    6 FORMAT(1X,'TOTAL NUMBER OF RESERVOIRS: ',I3)
      ELSE
      CALL USTOP('NUMBER OF RESERVOIRS LESS THAN 1')
      ENDIF 
C
...
C2C1----IF ILLEGAL PRINT A MESSAGE AND ABORT SIMULATION
      CALL USTOP('RES: ILLEGAL OPTION NRESOP= '//NUM2STR(NRESOP))
C
...
C
      DO N=1, NRES
        ...
        IF    (EOF) THEN
            CALL USTOP('RES PACKAGE INPUT REACHED END, '//
     +                 'BUT SIMULATION NOT COMPLETE')
        ELSEIF(NOFREE) THEN
            READ(LINE,"(2F10.0)") HRESSE(1,N),HRESSE(2,N)
        ELSE
            LLOC=1
            CALL GET_NUMBER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,HRESSE(1,N),
     +                               MSG='MNWI FAILED TO READ Ststage')
            CALL GET_NUMBER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,HRESSE(2,N),
     +                               MSG='MNWI FAILED TO READ Endstage')
        END IF
      !
      END DO
C
