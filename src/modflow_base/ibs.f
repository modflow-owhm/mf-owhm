...
C1------IDENTIFY PACKAGE.
        WRITE(IOUT,1)IN
    1 FORMAT(1H0,'IBS -- INTERBED STORAGE PACKAGE, VERSION',
...
C1b------ ARE USED.
  ...
  104  FORMAT(1H0,'***ERROR*** THE IBS AND SUB PACKAGE SHOULD ',
     ...
     2     'THE SAME SIMULATION. ********STOPPING******** ')
       CALL USTOP(
     1 'THE IBS AND SUB PACKAGE CANNOT BE USED AT THE SAME TIME ')
      ENDIF
C
...
C4------CRITICAL HEAD ARRAYS.
     ...
     +          BACK_UP=.TRUE., HED="-- READING IBS PACKAGE INPUT --")
      !
      IF(NOFREE) THEN
