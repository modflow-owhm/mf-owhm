...
C
      ...
      CTEXTH = '  GHOST-NODE HEAD'
      PTEXTF = '  GHOST-NODE FLUX'
C     ------------------------------------------------------------------
...
C1------PRINT A MESSAGE IDENTIFYING BFH PACKAGE
  ...
  500 FORMAT(1X,/1X,'BFH -- BOUNDARY FLOW AND HEAD PACKAGE',
     &    ', VERSION 2.0 - OWHM',/8X,'INPUT READ FROM UNIT ',I4)

C2------CHECK IF LGR IS ACTIVE, IF SO STOP.
      IF(ILGR .NE. 0)THEN 
        CALL USTOP('THE BFH PACKAGE CANNOT BE USED WHEN RUNNING LGR'//
