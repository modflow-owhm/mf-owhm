...
C4------CHECK TO SEE THAT OPTION IS LEGAL.
      IF(NRCHOP.LT.-1.OR.NRCHOP.GT.3.OR.NRCHOP==0) THEN
!          WRITE(IOUT,8) NRCHOP
...
C5------OPTION IS LEGAL -- PRINT OPTION CODE.
        IF(NRCHOP.EQ.-1) WRITE(IOUT,200)
  200   FORMAT(1X,
     1     'OPTION 1 -- RECHARGE TO INITIAL UPPER MOST ACTIVE LAYER')
...
C2------DETERMINE WHICH RECHARGE OPTION.
      IF(NRCHOP.EQ.-1) THEN
       ...
       DO IC=1,NCOL
        !
        IL=IRCH(IC,IR)
        IF(IBOUND(IC,IR,IL) > 0) RHS(IC,IR,IL)=RHS(IC,IR,IL)-RECH(IC,IR)
       END DO
       END DO
      ELSE IF(NRCHOP.EQ.1) THEN
C
...
C5B-----RIGHT-HAND-SIDE AND MOVE TO NEXT HORIZONTAL LOCATION.
        IF(IBOUND(IC,IR,IL).GT.0) THEN                                  ! NWT does not IBOUND=0 cells. Add Head Check when requesting to place water at upper most active
          HHNEW=REAL(HNEW(IC,IR,IL),KIND(HHNEW))
          BOT=BOTM(IC,IR,LBOTM(IL))
          IF(LAYHDT(IL).GT.0 .AND. HHNEW.LT.BOT) GO TO 28               
          RHS(IC,IR,IL)=RHS(IC,IR,IL)-RECH(IC,IR)
          GO TO 30
        END IF
   28   CONTINUE
