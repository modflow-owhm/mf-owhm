...
C3------READ NRCHOP AND IRCHCB.
     ...
     +                          HED="-- READING RCH PACKAGE INPUT --")
      ...
      ! CHECK IF GLOBAL SHUTDOWN OF CBC IS IN EFFECT
       CALL CHECK_CBC_GLOBAL_UNIT(IRCHCB, .FALSE.)
      !
C
C4------CHECK TO SEE THAT OPTION IS LEGAL.
      IF(NRCHOP.LT.1.OR.NRCHOP.GT.5) THEN
        CALL USTOP(' ILLEGAL RECHARGE OPTION CODE: NRCHOP = '//
     1             NUM2STR(NRCHOP)//'SUPPORTED VALUES ARE '//
     2             '1, 2, 3, 4, and 5')
      END IF
C
C5------OPTION IS LEGAL -- PRINT OPTION CODE.
        IF(NRCHOP.EQ.1) WRITE(IOUT,201)
  201 FORMAT(1X,'OPTION 1 -- RECHARGE TO LAYER 1')
  ...
  203 FORMAT(1X,'OPTION 3 -- RECHARGE TO HIGHEST ACTIVE NODE IN',
     1     ' EACH VERTICAL COLUMN (UPPER MOST IBOUND /=0 LAYER). ',
     2     'NWT SOLVER WITH CONVERTIBLE LAYER CAN APPLY WATER TO A',
     2     '"DRY" CELL, BUT IT WILL DRAIN DOWNWARD TO THE WATER TABLE.')
        IF(NRCHOP.EQ.4) WRITE(IOUT,205)
  205 FORMAT(1X,'OPTION 4 -- RECHARGE TO WATER TABLE NODE IN',
     1     ' EACH VERTICAL COLUMN CONVERTIBLE LAYERS AND  ',
     1     'CONFINE LAYERS GO TO HIGHEST ACTIVE NODE.')
        IF(NRCHOP.EQ.5) WRITE(IOUT,206)
  206 FORMAT(1X,'OPTION 5 -- RECHARGE TO WATER TABLE NODE IN',
     1     ' EACH VERTICAL COLUMN FOR BOTH CONFINE AND ',
     1     ' CONVERTIBLE LAYERS.')
C
...
C7------ARRAYS.
      ...
      ALLOCATE (IRCH(NCOL,NROW), SOURCE=1)
C
...
C2------READ FLAGS SHOWING WHETHER DATA IS TO BE REUSED.
      ...
      ELSE
          LLOC = 1
          CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,INRECH,
     &           MSG='RCH PACKAGE FAILED TO READ INRECH')
          IF(NRCHOP == 2) THEN
             CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,INIRCH,
     &           MSG='RCH PACKAGE HAS NRCHOP == 2, WHICH READS A '//
     &           'SECOND INTEGER INPUT (INIRCH) FOR EACH STRESS PERIOD')
          END IF
      END IF
C
...
C3B2----PARAMETERS TO USE THIS STRESS PERIOD.
   ...
   33     FORMAT(1X,///1X,
     1      'RECH array defined by the following parameters:')
          IF(INRECH.EQ.0) THEN
                WRITE(LINE,'(A)')' When parameters are defined for '//
     &      'the RCH Package, at least one parameter'//NEW_LINE(" ")//
     &      ' must be specified each stress period (GWF2RCH7RPLL)'
              CALL USTOP(LINE)
          END IF
...
!     1        '  ROW',I4,'  :',I4)
            CALL USTOP('RCH PACKAGE HAS INVALID LAYER NUMBER IN IRCH '//
     &                 'FOR COLUMN '//NUM2STR(IC)//
     &                 ' ROW '//NUM2STR(IR)//' HAS LAY =  '//
     &                 NUM2STR(IRCH(IC,IR)) )
          END IF
          END DO
          END DO
...
C5B-----RIGHT-HAND-SIDE AND MOVE TO NEXT HORIZONTAL LOCATION.
   ...
   28   CONTINUE
   30   CONTINUE
      ELSE !IF(NRCHOP.EQ.4 .or. NRCHOP.EQ.5) THEN
C
C5------NRCHOP IS 3, RECHARGE IS INTO WATER TABLE CELL, EXCEPT
C5------CANNOT PASS THROUGH CONSTANT HEAD NODE
C5------TO MIMIC MF-NWT, UPPER MOST ACTIVE CELL IS SELECTED
        DO 40 IR=1,NROW
        DO 40 IC=1,NCOL
...
C
C5A-----IF CELL IS CONSTANT HEAD MOVE ON TO NEXT HORIZONTAL LOCATION.
        IF(IBOUND(IC,IR,IL).LT.0) GO TO 40
C
C5B-----IF THE CELL IS VARIABLE HEAD, SUBTRACT RECHARGE FROM
C5B-----RIGHT-HAND-SIDE AND MOVE TO NEXT HORIZONTAL LOCATION.
        IF(IBOUND(IC,IR,IL).GT.0) THEN
          ...
          IF(HHNEW.LT.BOT) THEN                                 ! Head Check when requesting to place water at upper most active
              IF(LAYHDT(IL).GT.0 .OR. NRCHOP.EQ.5) GO TO 38
          END IF
          RHS(IC,IR,IL)=RHS(IC,IR,IL)-RECH(IC,IR)
          GO TO 40
        END IF
...
C
C7B-----IF CELL IS CONSTANT HEAD, MOVE ON TO NEXT HORIZONTAL LOCATION.
        IF(IBOUND(IC,IR,IL).LT.0) GO TO 39
C
C7C-----IF CELL IS VARIABLE HEAD, THEN DO BUDGET FOR IT.
        IF (IBOUND(IC,IR,IL).GT.0) THEN
          HHNEW=REAL(HNEW(IC,IR,IL),KIND(HHNEW))
