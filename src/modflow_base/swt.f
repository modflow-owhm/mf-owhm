...
C     ------------------------------------------------------------------
     ...
     4 ISWOCF,ISWOCU,IFL2,NOPCS
      ...
      ANAME(1)  = ' SILT AND CLAY THICKNESS'
      ANAME(2)  = 'ELASTIC SPECIFIC STORAGE'
      ANAME(3)  = 'INELAS. SPECIFIC STORAGE'
      ANAME(4)  = '              VOID RATIO'
      ANAME(5)  = '     PRECONSOL.   STRESS'
      ANAME(6)  = '        GEOSTATIC STRESS'
      ANAME(7)  = '   ELEV. OF LAYER CENTER'
      ANAME(8)  = '     STARTING COMPACTION'
      ANAME(9)  = '   ELEV. OF LAND SURFACE'
      ANAME(10) = '  MOIST SPECIFIC GRAVITY'
      ANAME(11) = '   SAT. SPECIFIC GRAVITY'
      ANAME(12) = '     RECOMPRESSION INDEX'
      ANAME(13) = '       COMPRESSION INDEX'
      TEXT(1)   = 'CENTER ELEVATION'
...
C1------IDENTIFY PACKAGE.
        WRITE(IOUT,1)IN
    1 FORMAT('SWT -- SUBSIDENCE FOR WATER-TABLE PACKAGE, VERSION',
     1     ' OWHM',' INPUT READ FROM UNIT',I3)
C
...
C
      ...
      DO NS=1, NPER
       ...
       IF(ISSFLG(NS).NE.0.AND.NS.GT.1) THEN
        CALL USTOP('SWT CAN ONLY HAVE THE FIRST STRESS PERIOD'//
     1  ' WITH STEADY-STATE (SS) OR TRANSIENT (TR). ALL OTHER STRESS '//
     2  ' PERIODS MUST BE TRANSIENT (TR). PLEASE FIX THE DIS PACKAGE.')
       ENDIF
      enddo
C
C3------Check that there are no quasi-3d confining beds
      IF(NCNFBD.GT.0) THEN
        CALL USTOP('QUASI-3D confining beds cannot '//
     &             'be used with SWT')
      END IF
C
...
C4------FLAG FOR PRINTING AND STORING COMPACTION, SUBSIDENCE, AND
C4------CRITICAL HEAD ARRAYS.
      CALL CHECK_CBC_GLOBAL_UNIT(ISWTCB, .TRUE.)
C
...
C ------ABORT IF NO LAYERS ARE SPECIFIED FOR INTERBED STORAGE
      IF(NSYSTM.LT.1) THEN
       CALL USTOP('SWT - NO LAYERS WITH INTERBED STORAGE '//
     1            'WERE SPECIFIED IN INPUT.')
      ENDIF
...
C ------FOR LAYERS WITHOUT DELAY.
  ...
  116  FORMAT(/,' MODEL LAYER ASSIGNMENTS FOR EACH OF',I3,' SUB-WT',
     1  ' SYSTEMS OF INTERBEDS:')
      ...
      LLOC=1
      CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,LNWT(1:NSYSTM),
     +                 MSG="SWT ERROR READING LNWT(NSYSTM)")
       !READ(LINE,*) (LNWT(N),N=1,NSYSTM)
         WRITE(IOUT,117) (LNWT(N),N=1,NSYSTM)
  117  FORMAT(1X,25I4)
       DO N=1,NSYSTM
       IF(LNWT(N).GE.1.AND.LNWT(N).LE.NLAY) CYCLE
       CALL USTOP('SWT - IMPROPER LAYER ASSIGNMENT FOR SUB-WT '//
     1            'SYSTEM OF INTERBEDS')
       ENDDO
      DO K=1,NLAY
C ------MAKE SURE THERE ARE NO QUASI-3D CONFINING LAYERS
      IF(LAYCBD(K).NE.0) THEN
       CALL USTOP('SWT CANNOT BE USED IN CONJUNCTION WITH QUASI-3D '//
     &            'CONFINING UNITS')
      ENDIF
      ENDDO
...
C7------thicknesses
   ...
   44     FORMAT(' STOPPING-- zero or negative layer thickness ',/,
     &   ' found at (row, column, layer):',3I5)
         ...
         CALL USTOP('SWT ZERO OR NEGATIVE LAYER THICKNESS. '//
     +              'PLEASE SEE LIST FILE.')
        ENDIF
      ENDDO
      ENDDO
      ENDDO
...
C ------READ FLAGS AND FORMATS FOR PRINTING CALCULATED ARRAYS
      ...
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IPCSFL,R,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IPCSFM,R,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,ISTFL,R,IOUT,IN) 
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,ISTFM,R,IOUT,IN) 
C
...
C
  ...
  502  FORMAT(1X,'ONLY ',I4,' OUT OF ',I4,' OUTPUT CONTROL RECORDS ',
     1  'FOR SUB-WT WERE FOUND.',/,1X,'SIMULATION ABORTED.')
       CALL USTOP('SWT OUTPUT CONTROL ERROR. PLEASE SEE LIST.')
C
C6------RETURN
...
C
    ...
    5  FORMAT(' NEGATIVE EFFECTIVE STRESS VALUE AT (ROW,COL,LAY):',
     $ 3I5,/,'   ABORTING...')
      CALL USTOP('NEGATIVE EFFECTIVE STRESS VALUE. PLEASE SEE LIST.')
      ENDIF
      ENDDO
      ENDDO
      ENDDO
