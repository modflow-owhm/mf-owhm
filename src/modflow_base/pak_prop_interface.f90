MODULE PAK_PROP_INTERFACE
  ...
  USE ERROR_INTERFACE,        ONLY: STOP_ERROR, FILE_IO_ERROR, GET_WARN
  USE FILE_IO_INTERFACE,      ONLY: READ_TO_DATA, COMMENT_INDEX
  USE PARSE_WORD_INTERFACE,   ONLY: PARSE_WORD, PARSE_WORD_UP
  USE STRINGS,                ONLY: UPPER, GET_INTEGER, GET_NUMBER
  USE NUM2STR_INTERFACE,      ONLY: NUM2STR
  USE GENERIC_OPEN_INTERFACE, ONLY: GENERIC_OPEN
  IMPLICIT NONE
  !
  PRIVATE:: Z, ONE, TRUE, FALSE, NL
  PRIVATE:: STOP_ERROR, FILE_IO_ERROR, GET_WARN
  PRIVATE:: READ_TO_DATA, COMMENT_INDEX
  PRIVATE:: PARSE_WORD, PARSE_WORD_UP
  PRIVATE:: UPPER, GET_INTEGER, GET_NUMBER
  PRIVATE:: NUM2STR
  PRIVATE:: GENERIC_OPEN
...
END SUBROUTINE
!
SUBROUTINE ULSTRDSTRUCT(NLST,RLST,LSTBEG,LDIM,MXDIM,INPACK,IOUT,           &
...
! MXDIM: SIZE OF RLIST (i.e. RLIST(MXDIM))
...
!     ******************************************************************
  ...
  INTEGER, INTENT(IN)::NLST,LSTBEG,LDIM,MXDIM,INPACK,IPRFLG
  INTEGER, INTENT(IN)::IOUT,NAUX,NCAUX,IFREFM,ISCLOC1,ISCLOC2
  CLASS(PAK_PROP), DIMENSION(MXDIM), INTENT(INOUT):: RLST
  CHARACTER(*),                      INTENT(IN   ):: LABEL
  CHARACTER(16), DIMENSION(NCAUX),   INTENT(IN   ):: CAUX
  !DIMENSION RLIST(LDIM,MXDIM)
...
!          TEST IF OPEN\CLOSE FILE EXISTS
         ...
         ELSE
             ...
             IF (IBINARY.NE.ONE) CALL READ_TO_DATA(LINE, IN, NOSHIFT=NOSHIFT)
             !
             CALL DATAFILE_UNIT_NUMBER%ADD(IN)
         END IF
  END IF
!
...
!     ------------------------------------------------------------------
  USE PARAMMODULE
  INTEGER, INTENT(IN   ):: IOUTU,IN,LDIM,MXDIM,MXLST
  INTEGER, INTENT(INOUT):: NTOT
  INTEGER, INTENT(IN   ):: IPVL1,IPVL2,NCAUX,NAUX,IPRT
  CLASS(PAK_PROP),DIMENSION(MXDIM),INTENT(INOUT)::RLST
  CHARACTER(*), INTENT(IN):: LABEL
  CHARACTER(16),DIMENSION(NCAUX), INTENT(IN):: CAUX
  CHARACTER(*), INTENT(IN):: PACK,PTYP
