  ...
  SUBROUTINE BUDGET_GROUPS_LOAD( BD,  BL )
    ...
    IF( BL%NAME == 'BUDGET_GROUP' .OR. BL%NAME == 'BUDGET_GROUPS') THEN
       IF( BL%NLINE>Z ) THEN
           ...
           !
           IF(FL%IS_CONSTANT) CALL STOP_ERROR(LINE=BL%LINE, INFILE=BL%IU, OUTPUT=BL%IOUT,MSG='BUDGET_GROUP BLOCK ERROR: CONSTANT FILE IO DIRECTIVE IS NOT SUPPORTED.')
           !
           IF(FL%IS_INTERNAL) THEN  ! INTERNAL KEYWORD FOUND, MOVE ONE LINE DOWN
               CALL BL%NEXT()
           ELSEIF (FL%IU /= Z) THEN ! FOUND "EXTERNAL" OR "OPEN/CLOSE".
