...
MODULE SFR_INPUT_DATA_TYPES!, ONLY: SFR_NAMED_LOCATION, SFR_SEGRCH_TFR, SFR_SEG_TFR, FMP_FLOW_REMOVE_SFR
  ...
  USE STRINGS,                          ONLY: JOIN_TXT, GET_WORD, GET_INTEGER, GET_NUMBER
  USE POSITION_INTERFACE,               ONLY: STR_POS
  USE UTIL_INTERFACE,                   ONLY: 
  ...
  SUBROUTINE LOAD_SFR_NAMED_LOCATION_BYLINE(SFR, IU, IOUT, LINE, LLOC, ISTART, ISTOP, INTERNAL_COUNT)
    ...
    !
    IF(FL%IU == Z) THEN  ! Found implied internal
        IFIL = IU
    ELSE
        IFIL = FL%IU
    END IF
  ...
  SUBROUTINE SFR_NAMED_LOCATION_LINE_TO_WBSSEGRCH(SFR, LINE, LLOC, ISTART, ISTOP, IOUT, IN, WBS, SEG, RCH, NOID)
    ...
    END IF
    !
    CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,WBS, MSG = 'FAILED TO LOCATE THE WBS/FID NUMBER ON LINE')
    !
    IF(WBS > Z) THEN
        !
  ...
  SUBROUTINE ALLOCATE_FMP_FLOW_REMOVE_SFR(SFR,DIM)
    ...
    SFR%N   = Z
    !
    ALLOCATE(SFR%ISTRM(DIM),         SOURCE= Z)
    ALLOCATE(SFR%INFLOW(DIM),        SOURCE=DZ)
    ALLOCATE(SFR%REMOVE_INFLOW(DIM), SOURCE=DZ)
    !
  END SUBROUTINE
  !
  ...
  PURE SUBROUTINE SORT_ISTRM_FMP_FLOW_REMOVE_SFR(SFR)
    CLASS(FMP_FLOW_REMOVE_SFR),  INTENT(INOUT):: SFR
    !
    IF(SFR%N>ONE) CALL SORT( SFR%ISTRM(1:SFR%N) )
    !
  END SUBROUTINE
  !
