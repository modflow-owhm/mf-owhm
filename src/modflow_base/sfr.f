...
C       SEG(9 ,NSEG)=
C       SEG(10,NSEG)=
C       SEG(11,NSEG)= ( icalc.EQ.1 ) READ (In, *, IOSTAT=I) (SEG(jj, nseg), jj=11, 14), (SEG(jj, nseg), jj=22, 24)
C       SEG(12,NSEG)=
C       SEG(13,NSEG)=
C       SEG(14,NSEG)=
C       SEG(15,NSEG)=
...
C       SEG(27,NSEG)= Holds SEGMENT Flow  limit; Originally used by Rich Niswonger for debugging; comment -> "SNTEMP GW flows by segment"
C       SEG(28,NSEG)= Holds SEGMENT Stage limit; Originally used to hold LGR inflow from parent grid, but was immediately set to SEG(2,NSEG)
C
C       ISEG(1 ,NSEG)= ICALC
...
C      
C      !
C      WRITE(321,'(A)')"ISTRM XXX"
C      DO I = 1, NSTRM 
C          WRITE(321,'(I6, 5x, *(2x, I6))')  i, ISTRM(:,I)
C      END DO
C      !
C      WRITE(321,'(A)')"ISEG"
C      DO nseg = 1, NSS
C          WRITE(321,'(I6, 5x, *(2x, I6))')  nseg, ISEG(:,nseg)
C      END DO
C      !
C      WRITE(321,'(A)')"IOTSGZ XXX"
C      DO nseg = 1, NSS
C          WRITE(321,'(I6, 5x, *(2x, I6))')  nseg, IOTSG(nseg)
C      END DO
C      !
C      WRITE(321,'(A)')"IDIVAR XXX"
C      DO nseg = 1, NSS
C          WRITE(321,'(I6, 5x, *(2x, I6))')  nseg, IDIVAR(:,nseg)
C      END DO
C      !
C      WRITE(321,'(A)')"SEG XXX"
C      DO nseg = 1, NSS
C          WRITE(321,'(I6, 5x, *(1x, ES13.7E1))')  i, SEG(:,nseg)
C      END DO
C      !
C      WRITE(321,'(A)')"STRM XXX"
C      DO I = 1, NSTRM 
C          WRITE(321,'(I6, 5x, *(1x, ES13.7E1))')  i, STRM(:,I)
C      END DO
C      !ISTRM(6,NSTRM) ISEG(4 ,NSEG) IOTSG(NSEG) IDIVAR(1,NSEG) STRM(31,NSTRM) SEG(28,NSEG) 
C      stop 0
C
C     ******************************************************************
...
C     ******************************************************************
      ...
      USE CONSTANTS,      ONLY: NL,BLNK,Z,ONE
      IMPLICIT NONE
      ...
      IF ( UHC > 1.0e-20 ) THEN
      IF ( self%LTYPE > Z  .AND. IBOUND(JRCH,IRCH,KRCH) > Z ) THEN 
        IF ( STRM(4, self%IRCHNUM)-BOTM(JRCH,IRCH,LBOTM(KRCH))
     +                                      .LT.-1.0E-12 ) THEN
          IF ( self%IFLAG == Z ) THEN
          WRITE(self%IUNIT,*)
     ...
     +                STRM(4, self%IRCHNUM),BOTM(JRCH,IRCH,LBOTM(KRCH))
      ...
      END IF
      END IF
      IF ( self%IFLAG > Z .AND. self%IRCHNUM.EQ.NSTRM ) THEN
...
C-------SUBROUTINE GWF2SFR7AR
      SUBROUTINE SFR_MF2005_OPTIONS(LINE, HAS_OPT, IN, IFACE, IRFG)
      USE GWFSFRMODULE
      USE CONSTANTS,            ONLY: NL, TRUE, FALSE, DZ, UNO, Z, ONE
      ...
      LOGICAL,      INTENT(INOUT):: HAS_OPT
      INTEGER,      INTENT(IN   ):: IN
      INTEGER,      INTENT(INOUT):: IFACE, IRFG
      ...
      DO 
        ...
        CASE('NOPRINT', 'NO_PRINT')
            SFR_PRNT = FALSE
            !
        CASE('NO_REACH_LAYER_CHANGE') ! UPLAY_ADJUST => 0 do not adjust layers, 1 adjust layer if deeper, 2 use current upper most active layer
            UPLAY_ADJUST = Z
            WRITE(IOUT,'(/3A/)')'NO_REACH_LAYER_CHANGE Keyword found.',
     +                         'Stream Reach will remain the same ',
     +                         'as specified in the input.'
            !
        CASE('REACH_LAYER_CHANGE_DEEPER')
            UPLAY_ADJUST = ONE
           WRITE(IOUT,'(/3A/)')
     +                   'REACH_LAYER_CHANGE_DEEPER Keyword found.',
     +                   'Stream Reach will only be changed if active ',
     +                   'layer is deeper then input specified.'
            !
        CASE('REACHINPUT')
           IRFG = ONE
     ...
     +                    'IFACE values will be read from reach records'
        CASE('TABFILES')
            CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,NUMTAB,
     +      MSG='SFR ERROR LOADING TABFILES "NUMTAB"')
            CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,ROWTAB,
     +      MSG='SFR ERROR LOADING TABFILES "ROWTAB"')
            IF(NUMTAB < Z) NUMTAB = Z
            IF(ROWTAB < Z) ROWTAB = ONE
     ...
     +                 'Multiplication factor is equal to ',FACTOR
        CASE('AUX','AUXILIARY')
            CALL PARSE_WORD_UP(LINE,LLOC,ISTART,ISTOP)
            IF(LINE(ISTART:ISTOP) == "IFACE") THEN
              IFACE = ONE
              WRITE(IOUT,'(/A/)') 'IFACE KEYWORD FOUND. '//NL//
     +                    'IFACE values will be read from reach records'
            ELSEIF(NSFRAUX.LT.20) THEN
               NSFRAUX=NSFRAUX+1
     ...
     +                 'SFR AUXILIARY (AUX) STREAM FLOW ROUTING '//
     +                 'VARIABLES ARE SPECIFEID MORE THAN 20 TIMES.' )
            END IF
            !
        CASE('PRINT_GW_FLOW_RESIDUAL') ! OUTER_START NTERM FILE
            ...
            !
            CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,
     +                       SFR_FRES_NTERM,MSG=
     ...
     +  'BUT FAILED TO LOAD THE STARTING OUTER ITERATION (OUTER_START)')
         !
         CALL SFR_FRES%OPEN(LINE,LLOC,IOUT,IN,
     +                       NO_INTERNAL=TRUE)
         !
         IF(.NOT. SFR_FRES%BINARY)
     +   CALL SFR_FRES%SET_HEADER('   SP   TS  ITER  LAY  ROW  COL  ' //
     +   'SEG  RCH            HEAD     HEAD_STREAM  '                 //
     +   'ELEV_UP_STREAM    WIDTH_STREAM         '         //
     +   'FLOW_IN        FLOW_OUT    FLOW_SEEPAGE          '      //
     +   'RUNOFF     COND_STREAM   FLOW_RESIDUAL    '      //
     +   'VOL_RESIDUAL     CELL_VOLUME      DATE  CELL_ID')
         !
         IF(SFR_FRES_NTERM < ONE) SFR_FRES_NTERM = ONE
         ALLOCATE(SFR_FRES_LRC(SFR_FRES_NTERM) ,
     +            SFR_FRES_DIF(SFR_FRES_NTERM)  )
        CASE('PRINT_SEGMENT_GW_FLOW_RESIDUAL') ! OUTER_START NTERM FILE
            !
       WRITE(IOUT,'(A)')' FOUND OPTION "PRINT_SEGMENT_GW_FLOW_RESIDUAL"'
            !
            CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,
     +                       SFR_SEG_FRES_OUTER,MSG=
     +  'FOUND SFR OPTION "PRINT_SEGMENT_GW_FLOW_RESIDUAL"'//NL//
     +  'BUT FAILED TO LOAD THE STARTING OUTER ITERATION (OUTER_START)')
         !
         CALL SFR_SEG_FRES%OPEN(LINE,LLOC,IOUT,IN,
     +                       NO_INTERNAL=TRUE)
         !
         IF(.NOT. SFR_SEG_FRES%BINARY)
     +       CALL SFR_SEG_FRES%SET_HEADER(
     +   '   SP   TS  ITER  LAY  ROW  COL  SEG  RCH            ' //
     +   'HEAD     HEAD_STREAM  ELEV_UP_STREAM    '              //
     +   'WIDTH_STREAM         FLOW_IN        FLOW_OUT    '   //
     +   'FLOW_SEEPAGE          RUNOFF     COND_STREAM   '       //
     +   'FLOW_RESIDUAL      DATE  CELL_ID')
         !
        CASE('DBFILE') 
            WRITE(IOUT,'(2A)') 'DBFILE OPTION FOUND. ',
     +       'SFR INFORMATION WRITTEN TO DATABASE FRIENDLY OUTPUT'
            CALL DBFILE%OPEN(LINE,LLOC,IOUT,IN,SPLITMAXCOUNT=11,
     +                       NO_INTERNAL=.TRUE.,ALLOW_ONLY_UNIT=.TRUE.)
     ...
     +   'CHNG_UNSAT_STOR (double)'
        ...
        CASE('HEAD_RELAX','HEAD_RELAXATION')
            WRITE(IOUT,'(2A)') 'HEAD_RELAXATION OPTION FOUND. ',
     +       'NOW READING 0 to 1 HNEW1 TO HNEW2 RELAXATION FACTOR'
            CALL GET_NUMBER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,
     +                       HD_RELAX,MSG=
     +  'FOUND SFR OPTION "HEAD_RELAXATION" OR "HEAD_RELAX"'//NL//
     +  'BUT FAILED TO THE RELAXATION FACTOR (HD_RELAX)')
        CASE('HEAD_PREDICTOR_FACTOR', "HNEW_FACTOR")
            WRITE(IOUT,'(2A)') 'HEAD_PREDICTOR_FACTOR OPTION FOUND. ',
     +       'NOW READING 0 to 1 HNEW TO HOLD FACTOR'
            CALL GET_NUMBER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,
     +                       HNEW_FACTOR,MSG=
     + 'FOUND SFR OPTION "HEAD_PREDICTOR_FACTOR" OR "HNEW_FACTOR"'//NL//
     + 'BUT FAILED TO THE RELAXATION FACTOR (HNEW_FACTOR)')
        IF (HNEW_FACTOR > 0.99D0) HNEW_FACTOR = UNO
        IF (HNEW_FACTOR < 0.01D0) HNEW_FACTOR = DZ
        CASE DEFAULT
                    EXIT
        END SELECT
        !
      ...
      END DO
      !
      IF(HAS_OPT) CALL READ_TO_DATA(LINE,IN,IOUT)
...
!IFACE
      CHARACTER(8):: face
      INTEGER:: iface,ndash
      INTEGER:: lloc, istart, istop, nparseg, i, ii
      INTEGER:: nlst, lb, ichk, icalc
...
C     ------------------------------------------------------------------
      ...
      CALL ERR2%INIT()
      !
      IF(ALLOCATED(MX_STG)) DEALLOCATE(MX_STG)
      IF(ALLOCATED(MX_FLO)) DEALLOCATE(MX_FLO)
      !
      text    = 'SFR2'
      iterp   = ONE
      idum(1) = Z
      ...
      ALLOCATE(UPLAY_ADJUST, SOURCE=TWO)  ! 0 do not adjust layers, 1 adjust layer if deeper, 2 use current upper most active layer
...
C
      ...
      HAS_OPT = TRUE
      DO WHILE (HAS_OPT)
          CALL SFR_MF2005_OPTIONS(LINE, HAS_OPT, IN, IFACE, IRFG)
      END DO
      !
      ! ----------------------------------------------------------
      !
...
!     +         'PACKAGE SPECIFY A STARTING DATE WITH KEYWORD STARTDATE')
      ...
      ELSEIF(BL%NAME == 'OPTION' .OR. BL%NAME == 'OPTIONS' ) THEN
        ...
        CASE('MAX_STAGE')
            ALLOCATE( MX_STG, SOURCE = BL%LINE(LLOC:) )
        CASE('MAX_FLOW')
            ALLOCATE( MX_FLO, SOURCE = BL%LINE(LLOC:) )
            !
        CASE('NO_BUDGET_RECALCULATE_FLOW')
            DO_FM_BD = FALSE
            !
     ...
     +      MSG='SFR ERROR LOADING TABFILES "ROWTAB"')
            IF(NUMTAB < Z) NUMTAB=0
            IF(ROWTAB < Z) ROWTAB=1
            WRITE(IOUT,'(/A,A,I10,A,I10,A/)')
     +               'TABFILES (WITH AN S) KEYWORD FOUND', 
     +       'SFR Original TABFILES will be loaded and will be read ',
     +       NUMTAB,'with a maximum of ',ROWTAB,' row entries per file'
...
!     +           MSG='TOO MANY AUXILIARY ')
     ...
     +  'BUT FAILED TO LOAD THE STARTING OUTER ITERATION (OUTER_START)')
         !
         CALL SFR_FRES%OPEN(BL%LINE,LLOC,IOUT,BL%IU,
     +                       NO_INTERNAL=TRUE)
         !
         IF(.NOT. SFR_FRES%BINARY)
     +   CALL SFR_FRES%SET_HEADER('   SP   TS  ITER  LAY  ROW  COL  ' //
     ...
     +  'BUT FAILED TO LOAD THE STARTING OUTER ITERATION (OUTER_START)')
         !
         CALL SFR_SEG_FRES%OPEN(BL%LINE,LLOC,IOUT,BL%IU,
     +                       NO_INTERNAL=TRUE)
         !
         IF(.NOT. SFR_SEG_FRES%BINARY)
     +       CALL SFR_SEG_FRES%SET_HEADER(
     ...
     +               'THIS OPTION IS IGNORED AND NOT APPLIED.'//NL)
      ...
      ! ----------------------------------------------------------
      !
      IF(SFR_SEG_FRES%IS_OPEN .AND. SFR_FRES_NTERM == Z) THEN
          SFR_FRES_NTERM = ONE
         ALLOCATE(SFR_FRES_LRC(SFR_FRES_NTERM),
     +            SFR_FRES_DIF(SFR_FRES_NTERM)  )
      END IF
...
!      ELSEIF(SFRTABFILE%NTAB.EQ.0) THEN                    !seb
!           WRITE(IOUT,'(A)') ' No specifed inflow files (TABFILE)'
!      END IF
!      !
!      lloc = 1
...
!4b-----Data read for transient routing.
        IF ( ITRFLG == 1 ) THEN
          ...
          IF ( IRTFLG  > Z ) THEN
            ...
            IF ( WEIGHT < 0.0 .OR. WEIGHT.GT.1.0 ) WEIGHT=1.0
...
Cdep  allocate space for stream outflow derivatives for lake package
     ...
     +            TABTIME(ROWTAB,nssar+NUMTAB+1))
      ...
      SEG(27:28,:) = HUGE(SEG)
      !
      IF(ALLOCATED(MX_STG) .OR. ALLOCATED(MX_FLO)) THEN
         ALLOCATE(TMP_VEC(NSS))
         IF(ALLOCATED(MX_STG)) THEN
             LLOC = 1
             ii   = 0
             CALL ULOAD(TMP_VEC, LLOC, MX_STG, IOUT, IN, ii,
     +                  NO_INTERNAL=TRUE, MSG='SFR OPTION MAX_STAGE')
             SEG(27,:) = TMP_VEC
         END IF
         !
         IF(ALLOCATED(MX_FLO)) THEN
             LLOC = 1
             ii   = 0
             CALL ULOAD(TMP_VEC, LLOC, MX_FLO, IOUT, IN, ii,
     +                  NO_INTERNAL=TRUE, MSG='SFR OPTION MAX_FLOW')
             SEG(28,:) = TMP_VEC
         END IF
         !
         DEALLOCATE(TMP_VEC)
...
C6------PRINT INFORMATION THAT WAS READ.
     ...
     +                                                    HNEW_FACTOR
      IF ( ISFROPT == 1 ) WRITE (IOUT, 9004)
      IF ( ISFROPT.GE.2 ) WRITE (IOUT, 9005)
      IF ( ISTCB1 > Z ) WRITE (IOUT, 9006) ISTCB1
      IF ( ISTCB2 > Z ) WRITE (IOUT, 9007) ISTCB2
      IF ( DBFILE%IU /= Z ) WRITE (IOUT, 1234) DBFILE%IU
      IF ( IRTFLG > Z ) WRITE (IOUT, 9035)
 9003 FORMAT (//, ' NUMBER OF STREAM REACHES IS', I5, //, 
     ...
     +        //, ' HEAD_PREDICTOR_FACTOR (HNEW_FACTOR) IS', E12.4,
     +        ///)
...
C
C7------CHECK FOR ERRORS.
      IF ( NSFRPAR > Z .AND. nparseg <= Z ) THEN
     ...
     +        'SETTING VALUE TO ABS(NSTRAIL) TO MAKE POSITIVE.'//NL)
          NSTRAIL = ABS(NSTRAIL)
        END IF
        IF ( NSTRAIL == Z ) THEN
...
!IFACE
      ...
      ELSE IF ( ISFROPT == 1 ) THEN
        ndash=ndash+55
          WRITE (IOUT, 9014) face
      ELSE IF ( ISFROPT == 2 ) THEN
        ndash=ndash+90
          WRITE (IOUT, 9015) face
      ELSE IF ( ISFROPT == 3 ) THEN
        ndash=ndash+101
          WRITE (IOUT, 9016) face
      ELSE IF ( ISFROPT == 4 ) THEN
          WRITE (IOUT, 9013) face
      ELSE IF ( ISFROPT == 5 ) THEN
          WRITE (IOUT, 9013) face
      END IF
!IFACE
...
!IFACE
     ...
     +        '. ERROR ON LINE "'//SFR_PREV_LINE(IN)//'"'//BLN)
              ...
              !CALL USTOP(' ')
          END IF
        ELSE IF ( ISFROPT == 4 .OR. ISFROPT == 5 ) THEN
!IFACE
          IF (iface == 1) THEN
        READ(In,*,IOSTAT=I) krch, irch, jrch, jseg, ireach, STRM(1, ii),
     +                 ISTRM(6, ii)
        ...
        END IF
        !
        IF(I /= Z) CALL SFR_STOP(IN, IOUT)
        !
        IF ( IUZT == 1 ) THEN
            IF ( IBOUND(jrch, irch, krch) <= Z )
...
C        AND SPECIFIC YIELD WHEN UNSATURATED FLOW IS ACTIVE.
        IF ( ABS(ITRSS) == 1 ) THEN
          IF ( ISFROPT == 2 .OR. ISFROPT == 3 ) THEN
            IF ( Iunitlpf /= Z ) THEN
              THTR(ii) = THTS(ii) - SC2LPF(jrch, irch, krch)
     +                   /(DELR(jrch)*DELC(irch))
            ELSEIF ( Iunitupw /= Z ) THEN
              THTR(ii) = THTS(ii) - SC2UPW(jrch, irch, krch)
     +                   /(DELR(jrch)*DELC(irch))
            ELSE IF ( Iunitbcf /= Z ) THEN
     ...
     +                   /(DELR(jrch)*DELC(irch))
              ELSE
                kkrch = 0
                DO k = 1, krch
                IF(LAYCON(K) == 3 .OR. LAYCON(K) == 2)kkrch = kkrch + 1
                END DO
                THTR(ii) = THTS(ii) - SC2(jrch, irch, kkrch)
     +                   /(DELR(jrch)*DELC(irch))
              END IF
            ELSE IF ( Iunithuf /= Z ) THEN
              THTR(ii) = THTS(ii) - SC2HUF(jrch, irch)
            END IF
          END IF
        ELSEIF ( ISFROPT == 2 .OR. ISFROPT == 3 ) THEN
          THTR(ii) = 0.0
      END IF
      IF(SFR_PRNT) THEN
        IF ( ISFROPT == Z .AND. IFLG == Z ) THEN
!IFACE
          IF (iface == 1) THEN
     ...
     +                       STRM(1, ii)
          END IF
        ELSE IF ( ISFROPT == 1  .AND. IFLG == Z ) THEN
          IF (iface == 1) THEN
     ...
     +                       STRM(8, ii), STRM(6, ii)
          END IF
        ELSE IF ( ISFROPT == 2 .AND. IFLG == Z ) THEN
          IF (iface == 1) THEN
     ...
     +                       THTI(ii), THTR(ii), EPS(ii)
          END IF
        ELSE IF ( ISFROPT == 3 .AND. IFLG == Z ) THEN
          IF (iface == 1) THEN
     ...
     +                       THTI(ii), THTR(ii), EPS(ii), UHC(ii)
          END IF
        ELSE IF ( ISFROPT == 4 .OR. ISFROPT == 5  .AND. IFLG == Z ) THEN
          IF (iface == 1) THEN
              WRITE (IOUT, 9019) krch, irch, jrch, jseg, ireach,
     +                       STRM(1, ii), ISTRM(6,ii)
          ELSE
...
C         Number of reaches in segment added to ISEG
          ISEG(4, jseg) = ireach
          IF ( ISFROPT.LT.4.AND.ISFROPT > Z )THEN  
      ...
      IF ( IUZT == 1 ) THEN
        ...
        DO nseg = 1, NSS
          ...
          IF ( icalc == 1 .OR. icalc == 2 ) THEN
            IF ( ISFROPT == 4 .OR. ISFROPT == 5 ) THEN
              ksfropt = 1
          ...
          DO ii = 1, ISEG(4, nseg)
            IF ( icalc == 1 .OR. icalc == 2 ) THEN
              ...
              IF ( ksfropt == 1 ) THEN
                ...
                IF ( ISFROPT == 5 ) UHC(irch) = SEG(21, nseg)
     +               - (uhcslpe*dist)
              END IF
C
...
! RGN 5/8/09 Fixed calculation of THTR to include HUF
     ...
     +                         /(DELR(jrck)*DELC(irck))
                  ELSE IF ( Iunitbcf /= Z ) THEN
                    IF( LAYCON(krck) == Z ) THEN
                      THTR(irch) = THTS(irch) - SC1(jrck, irck, krck)
     +                         /(DELR(jrck)*DELC(irck))
                    ELSE
                      kkrch = 0
                      DO k = 1, krch
                        IF(LAYCON(K) == 3 .OR. LAYCON(K) == 2)
     +                   kkrch = kkrch + 1
                      END DO
     ...
     +                           /(DELR(jrck)*DELC(irck))
                  END IF
                END IF
              ELSEIF ( ISFROPT == 4 .OR. ISFROPT == 5 ) THEN
                THTR(irch) = 0.0
              END IF
C
...
C         SATURATED WATER CONTENT.
     ...
     +        NUM2STR(nseg)//' REACH '//NUM2STR(irch)//BLN)
...
!     +                            THTR(irch)
     ...
     +               INFILE=IN,OUTPUT=IOUT,TAIL=NL,INIT=TRUE)
      !
      IF(WRN2%RAISED) THEN
        IF(UPLAY_ADJUST == 1) THEN
           CALL WRN2%CHECK(HED='SFR found segment/reaches with an '//
     +          'input layer specified as IBOUND=0 or '//
     +          'Head < Model cell bottom elevation for a '//
     +          'convertible layer.'//BLN//
     +          'The following had their layer number lowered '//
     +          '(increased) to the upper most active layer.'//BLN//
     +          '   SEG    RCH    LAY    LAY_DEEPER',
     +          INFILE=IN,OUTPUT=IOUT,TAIL=NL,INIT=TRUE,NO_NL=TRUE)
        ELSE
           CALL WRN2%CHECK(HED='SFR found segment/reaches with an '//
     +        'input layer specified is different than the '//
     +        'upper most active layer.'//BLN//
     +        'The following SFR option can disable '//
     +        'SFR changing the stream reach layer that is '//
     +        'specified in the SFR input.'//BLN//
     +        '"NO_REACH_LAYER_CHANGE"'//BLN//
     +        'To only change the layer if the the upper most '//
     +        'active layer is deeper use the next SFR option.'//BLN//
     +        '"REACH_LAYER_CHANGE_DEEPER"'//BLN//
     +        'The following had their layer number changed:'//BLN//
     +        '   SEG    RCH    LAY    LAY_NEW',
     +        INFILE=IN,OUTPUT=IOUT,TAIL=NL,INIT=TRUE,NO_NL=TRUE)
        END IF
      END IF
...
Cdep  Revised to change ib loop counter
          lb = lstbeg
          DO i = ib, numinst
            IF ( i > Z ) CALL UINSRP(i, In, IOUT, ip, iterp)
            ichk = 0
...
C     ------------------------------------------------------------------
C
      IF ( Iunitlpf /= Z .OR. Iunithuf /= Z .OR. Iunitupw /= Z ) THEN 
        IF ( ISFROPT == 2.OR.ISFROPT == 4 )
     +    CALL SGWF2SFR7UHC(Iunitlpf, Iunithuf, Iunitupw)
! set streambed K when option to use KV or KH are defined
      if ( Iunitupw .NE. 0 .AND.
     +     ( STRHC1KHFLAG == 1 .OR.  STRHC1KVFLAG == 1) ) then
        DO ichk = 1, NSTRM
          krck = ISTRM(1, ichk)
          irck = ISTRM(2, ichk)
          jrck = ISTRM(3, ichk)
          IF ( IBOUND(jrck, irck, krck) == Z ) THEN
              IF ( STRHC1KHFLAG == 1 ) THEN
                  STRM(6,ichk) = FACTORKH*HKUPW(jrck,irck,krck)
              ELSE IF ( STRHC1KVFLAG == 1 ) THEN   
      ...
      IF(.NOT. DBFILE%BINARY) THEN 
         IF(    IUZT == Z .AND. DBFILE%IU /= Z) THEN                    !WRITE HEADER FOR DBFILE IF REQUESTED
     ...
     +                             )
            !
         ELSEIF(IUZT /= Z .AND. DBFILE%IU /= Z) THEN
            !
            CALL DBFILE%SET_HEADER( ' DATE_START               '//
     +   'PER     STP             DELT          '//
     +   'SIMTIME    SEG    RCH          FLOW_IN     FLOW_SEEPAGE'//
     ...
     +                           )
      ...
      DO I = 1, NSTRM 
          IF( jseg .NE. ISTRM(4,I)) THEN
              jseg = ISTRM(4,I)
              SEG_NSTRM(jseg) = I - ONE  !NRCH = SEG_NSTRM(jseg+1) - SEG_NSTRM(jseg) and ISTRM = SEG_NSTRM(jseg) + irch
          END IF
      END DO
...
C     ------------------------------------------------------------------
     ...
     +     wdthlw, width, updiff, zero, DDIFF
      INTEGER i, ic, icalc, ichk, icp, iflginit, ii, ik, il, ip,
...
C9------READ DATA ACCORDING TO VARIABLE ISFROPT.
     ...
     +         isoptflg = 1
          ...
          ELSE IF ( IDIVAR(2, nseg).EQ.-2 ) THEN
            IF ( SEG(2, nseg) < 0.0 .OR. SEG(2, nseg).GT.1.0 ) THEN
     ...
     +          BLNK//NUM2STR(rough,10)//'  = ROUGHCH')
          ...
          ELSE IF ( icalc.EQ.2 ) THEN
            ...
            IF ( roughch.LE.0.0 ) THEN
                CALL ERR%ADD(NUM2STR(nseg,6)//BLNK//NUM2STR(icalc,6)//
     +          BLNK//NUM2STR(roughch,10)//'  = ROUGHCH'//NL)
            ...
            IF ( roughbnk.LE.0.0 ) THEN
                CALL ERR%ADD(NUM2STR(nseg,6)//BLNK//NUM2STR(icalc,6)//
     +          BLNK//NUM2STR(roughbnk,10)//'  = ROUGHBK'//NL)
...
C         WHEN ICALC = 4.
     ...
     +               '  FIRST FLOW VALUE <0.1, RESET TO 0.1'//NL)
            ...
            IF ( dpthlw.LE.0.0 ) THEN
                CALL WRN%ADD(NUM2STR(nseg,6)//
     +               '  FIRST DEPTH VALUE <0.01, RESET TO 0.01'//NL)
            ...
            IF ( wdthlw.LE.0.0 ) THEN
                CALL WRN%ADD(NUM2STR(nseg,6)//
     +       '  FIRST WITH VALUE <=0.0, RESET TO 1.0 - CHECK INPUT'//NL)
     ...
     +               BLNK//NUM2STR(flw2,10)//'  BAD FLOWTAB'//NL)
                 !WRITE (IOUT, 9023) nseg, flw2, ipt
                 !CALL USTOP(' ')
              END IF
              IF ( dpth2.LE.dpth1 ) THEN
                CALL ERR%ADD(NUM2STR(nseg,6)//BLNK//NUM2STR(ipt,6)//
     +               BLNK//NUM2STR(dpth2,10)//'  BAD DPTHTAB'//NL)
     ...
     +              'TYPICALLY, STREAM WIDTH INCREASES WITH FLOW'//NL)
            END DO
          END IF
        END DO
C
...
Crth --Include adjustment to BOTM if linked to SUB
     ...
     +                          GSE(icp,irp)-SEG(13,ISTRM(4,irch)-1)
...
C30-----DETERMINE VARIABLES WHEN UNSATURATED FLOW IS ACTIVE.
      DO l = 1, NSTRM
        ...
        ic = ISTRM(3, l)
        !h = WTABLE(ic,ir)
        IF(UPLAY(ic,ir)>0) THEN
           IF(UPLAY_ADJUST == 1) THEN                                  ! UPLAY_ADJUST => 0 do not adjust layers, 1 adjust layer if deeper, 2 use current upper most active layer)
                            IF(il <  UPLAY(ic,ir)) il = UPLAY(ic,ir)
           ELSEIF(UPLAY_ADJUST == 2) THEN 
                            IF(il /= UPLAY(ic,ir)) il = UPLAY(ic,ir) 
           END IF
           h = HNEW(ic,ir,il)
        ELSE
            h = HDRY ! Dry ell so set to hdry
        END IF
        !
        HLDSFR(l) = h
        istsg = ISTRM(4, l)
...
! segment number, number of rows, unit number
            ...
            DO j = 1, numval
              LLOC = 1
              CALL READ_TO_DATA(LINE,iunitnum,IOUT)
     ...
     +                    iunitnum)
              ...
              IF ( TABFLOW(j,ISFRLIST(1,i)) < 0.0 ) THEN
 ...
 9032 FORMAT(5X,G20.13,1X,G20.13)
      !
      RETURN
...
C     *****************************************************************
     ...
     +                     NEARZERO_15, NEGNEARZERO_15
      USE NUM2STR_INTERFACE, ONLY: NUM2STR
      IMPLICIT NONE
      INTRINSIC IABS, ABS, DABS, MIN, DSQRT, REAL, SQRT
C     -----------------------------------------------------------------
C     SPECIFICATIONS:
C     -----------------------------------------------------------------
C     FUNCTIONS
...
!dep changed to allow allocation of QA and QB from Steady State
          ...
          END IF
          !
          NO_GW = UP <= 0                                            ! If UP == 0, then no gw flow
          !
          FIXED_HEAD_CELL = IBOUND(ic, ir, il) < Z
          !
          IF( UP > Z ) THEN
             WT1 = HNEW(ic, ir, il)
             WT0 = WT1 
             !
             IF(PRED > 0.99D0 .OR. FIXED_HEAD_CELL) THEN
                  h = WT1
             ELSE
                  WT0 = HOLD(ic, ir, il)
                  IF( ABS(WT0-HDRY) < NEARZERO_15 ) WT0 = WT1
                  !
                  h = (WT0*CORR + WT1*PRED)
             END IF
          ELSE
              WT1 = HDRY  ! no head to measure so set to HDRY)
              WT0 = HDRY
              h   = HDRY
          END IF
C
C4------DETERMINE STREAM SEGMENT AND REACH SLOPE.
          icalc = ISEG(1, istsg)
...
C5------CHECK IF FLOW IS COMING FROM ANOTHER GRID.  
     ...
     +            lgrseg = INT(SEG(28+i, istsg))
              ...
              IF ( lgrgrid.GT.0 ) THEN
                ...
                SEG(2,istsg) = GWFSFRDAT(lgrgrid)%SGOTFLW(lgrseg)  ! SEG(28,istsg)
              END IF
...
C9B-----ESTIMATE LAKE OUTFLOW FOR FIRST ITERATION OF SIMULATION.
              IF ( SEG(2, istsg) < NEARZERO_15 ) THEN
...
! Added code to test for BCF or LPF 11/19/07
          IF (ABS(hld-HDRY) < NEARZERO_15) hld = h
          avhc = STRM(6, l)
          sbdthk = STRM(8, l)
          hstr = depth + STRM(3, l)
...
C         ZERO WHEN NOT AN ACTIVE CELL.
          IF ( FIXED_HEAD_CELL .OR. NO_GW) THEN
            avhc = 0.0
            strleak = 0.0D0
            h = hstr
...
! RGN might need to correct depth for smooth width
            ...
            END IF
            IF(depth > MX_STG) depth = MX_STG   ! seb
            !
...
C29-----CALCULATE AN INITIAL ESTIMATE OF FLOW IN CHANNEL.
            IF ( flwmpt.LT.NEARZERO ) THEN
              ...
              width = 0.0D0
              wetperm = 1.0D0
              IF ( avhc.LE.NEARZERO_15 ) iskip = 1
            END IF
            IF ( iskip.EQ.0 ) THEN
     ...
     +                            depth, itstr, width, iprndpth, MX_STG)
              ELSE IF ( icalc.EQ.3 ) THEN
                depth = cdpth*(flwest**fdpth)
                width = awdth*(flwest**bwdth)
                IF(depth > MX_STG) THEN             ! seb
                   width = width * (MX_STG / depth)
                   depth = MX_STG
                END IF
                wetperm = width
...
C31-----ESTIMATE INITIAL ENDPOINTS.
            enpt1 = 0.0D0
            IF ( depth >= MX_STG ) THEN
                 depth  = MX_STG
                 enpt2  = MX_STG
            ELSEIF ( depth.GT.NEARZERO ) THEN
...
C         THAN TOLERANCE.
              IF(depthp > MX_STG) THEN
                 flobotp = flobotp * (MX_STG / depthp)
                 IF ( icalc.GE.2 ) THEN
                      widthp   = widthp   * (MX_STG / depthp)
                      wetpermp = wetpermp * (MX_STG / depthp)
                 END IF
                 depthp = MX_STG
                 IF ( itot > 30 ) err = 0.0D0  ! Bad stage kill search
              END IF
              !
...
C67-----ROUTE FLOW DOWN CHANNEL WHEN ACTIVE.
     ...
     +               Transient_bd, depthtr, MX_STG)
            ELSE 
              qd = 0.0
            END IF
            IF( qc > MX_FLO ) qc = MX_FLO
            IF( qd > MX_FLO ) qd = MX_FLO
            STRM(28,l) = qc
            STRM(27,l) = qd
            flowot = qd
            flowc  = qc
...
C68-----STREAMFLOW OUT EQUALS STREAMFLOW IN MINUS LEAKAGE.
            IF( flowc  > MX_FLO ) THEN
                flowc  = MX_FLO
                flwmpt = 0.5D0*flowc
                IF ( flobot > flowc ) flobot = flowc
            END IF
            flowot = flowc - flobot
            IF( flowot > MX_FLO ) flowot = MX_FLO
            qc = flowc
            qd = flowot
...
cdep  changed dbleak to -CLOSEZERO
              ELSE IF ( SUMLEAK(l)-flowc.LT.NEGNEARZERO_15 ) THEN
C
...
!      end if
                ...
                ELSE
                   RHS (ic,ir,il) = RHS (ic,ir,il) + cstrsmooth*CORR*WT0
                   HCOF(ic,ir,il) = HCOF(ic,ir,il) - cstrsmooth*PRED
                END IF
                !
              ELSE
C
...
!IFACEt
     ...
     +                     NEARZERO_15, NEGNEARZERO_15
      IMPLICIT NONE
      INTRINSIC REAL, ABS, IABS, DSQRT, DLOG10, SQRT, SNGL
C     ------------------------------------------------------------------
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
C     FUNCTIONS
...
C10-----OUTFLOW FROM LAKE COMPUTED IN LAKE PACKAGE
              IF ( SEG(2,istsg).LE.NEARZERO_15 ) THEN
                flowin = STROUT(istsg)
...
!           strleak = 0.0D0
            h = hstr
          END IF
          IF (.NOT. NO_GW) THEN                           !seb changed to follow ilay which is set to the last layer if all vertical model cells are IBOUND(i,j,:)=0
!          IF ( ilay.LE.NLAY .AND. IBOUND(ic, ir, il).GT.0 ) THEN
C
C27-----COMPUTE HEAD DIFFERENCE ACROSS STREAMBED.
            !h = HNEW(ic, ir, il)
            !hld = HLDSFR(l)
            !IF ( ABS(SNGL(hld)-HDRY).LT.CLOSEZERO .OR. hld==ninf) hld=h
            !IF ( irt.EQ.numdelt ) HLDSFR(l) = h
            IF ( h.LT.sbot ) THEN
              hdiff = hstr - sbot
              grad = (hdiff)/sbdthk
...
! RGN Fixed statement. Only call subroutine if no UZ flow.
          IF ( icalccheck.EQ.1 ) THEN                            
            IF ( h.LT.sbot .AND. .NOT. NO_GW) THEN        !seb ADDED  .AND. IBOUND(ic, ir, ilay).NE.0 to ensure that only active cells get UZ flow or set flobot=0
...
C37-----ADD RATES TO BUFFERS.
          IF ( icalccheck.EQ.1 .AND. iss.EQ.0 ) THEN
            IF ( irt.EQ.numdelt .AND. .NOT. NO_GW ) THEN  !seb ONLY PROCESS ACTIVE CELLS
...
C39-----ADD FLOBOT TO RATIN WHEN STREAM RECHARGES GROUND WATER.
     ...
     +                            .AND. IBOUND(ic, ir, il).NE.0) THEN !seb ADDED  .AND. IBOUND(ic, ir, ilay).NE.0 to ensure that only active cells
...
!     +                         2( "," 1x I6), 13( "," 1x ES16.9))')
              DB_OUT(1) = STRM(10, l)   !FLOW_IN
              DB_OUT(2) = STRM(11, l)   !FLOW_SEEPAGE
              DB_OUT(3) = STRM(9, l)    !FLOW_OUT
              DB_OUT(4) = runof+runoff  !RUNOFF
              DB_OUT(5) = precip        !PRECIP
              DB_OUT(6) = etstr         !STREAM_ET
              DB_OUT(7) = STRM(15, l)   !HEAD_STREAM
              DB_OUT(8) = h             !HEAD_AQUIFER
              DB_OUT(9) = depth         !DEPTH_STREAM
              DB_OUT(10) = STRM(5, l)   !WIDTH_STREAM
              DB_OUT(11) = STRM(1,l)    !LENGTH_STREAM
              DB_OUT(12) = grad         !HEAD_GRADIENT  
              DB_OUT(13) = STRM(16, l)  !COND_STREAM
              DB_OUT(14) = STRM(3, l)   !ELEV_UP_STREAM
              DB_OUT(15) = 0D0
              DB_OUT(16) = 0D0
            IF(DBFILE%BINARY) THEN
              WRITE(DBFILE%IU)
...
C     ------------------------------------------------------------------
      ...
      DOUBLE PRECISION Flow, Slope, Wetperm, Depth, Totwdth, MX_STG
C     ------------------------------------------------------------------
C     LOCAL VARIABLES
C     ------------------------------------------------------------------
      INTEGER i, iflg
...
C-------SUBROUTINE GWF2SFR7TBD
      SUBROUTINE GWF2SFR7TBD(Flow, Depth, Width, Nstrpts, Istsg, MX_STG)
C     *******************************************************************
...
C     -------------------------------------------------------------------
      ...
      DOUBLE PRECISION Flow, Depth, Width, MX_STG
C     -------------------------------------------------------------------
C     LOCAL VARIABLES
C     -------------------------------------------------------------------
...
C6------COMPUTE DEPTH AND WIDTH FROM LOG INTERPOLATION.
        ...
        !
        IF    ( dlgstg > HUND ) THEN               ! seb overflow protection
                dlgwth = dlgwth * (HUND / dlgstg)
                dlgstg = HUND
        ELSEIF( dlgwth  > HUND ) THEN
                dlgstg = dlgstg * (HUND / dlgwth)
                dlgwth = HUND
        END IF
        !
        Depth = DIEZ**dlgstg
...
C     ******************************************************************
     ...
     +                        CONCQ, CONCRUN, CONCPPT, NSEGCK, IOUT
...
C         STREAM.
      !
      ! seb do not calcualte unsaturated flow if underlying cells are all dry -- Rich deleted this?
      !
...
C     ******************************************************************
     ...
     +                        FLUXHLD2, FLUXB, THETAB, IOUT, ISTRM
      USE ERROR_INTERFACE,    ONLY: STOP_ERROR
      USE NUM2STR_INTERFACE, ONLY: NUM2STR
...
C3------INITIALIZE CONSTANTS.
     ...
     +                        Width, iprndpth, MX_STG)
      ...
      ELSE IF ( Icalc.EQ.3 ) THEN
        ...
        ac = depth*Width
      ELSE IF ( Icalc.EQ.4 ) THEN
        CALL GWF2SFR7TBD(Qa, depth, Width, ISEG(2,Istsg), Istsg, MX_STG)
        aa = depth*Width
        CALL GWF2SFR7TBD(Qb, depth, Width, ISEG(2,Istsg), Istsg, MX_STG)
        ab = depth*Width
        CALL GWF2SFR7TBD(Qc, depth, Width, ISEG(2,Istsg), Istsg, MX_STG)
        ac = depth*Width
      END IF
C
...
C6------CALCULATE VARIABLES ad1 and ad2 ON BASIS OF Icalc.
     ...
     +                        Width, iprndpth, MX_STG)
          ...
          ELSE IF ( Icalc.EQ.4 ) THEN
        CALL GWF2SFR7TBD(Qd, depth, Width, ISEG(2,Istsg), Istsg, MX_STG)
            ad1 = depth*Width
        CALL GWF2SFR7TBD(qd2, depth, Width, ISEG(2,Istsg), Istsg,MX_STG)
            ad2 = depth*Width
        CALL GWF2SFR7TBD(qd3, depth, Width, ISEG(2,Istsg), Istsg,MX_STG)
          END IF
C
C7------CALCULATE FLOW.
...
C
     ...
     +                  Slope,Istsg,Nreach,Itstr,Width,depth,MX_STG)
C***********************************************************************
...
C     ------------------------------------------------------------------
     ...
     +   GWFSFRDAT(IGRID)%SFR_SEG_FRES%IS_OPEN) THEN
      ...
      SFR_FRES=>NULL()
      !
      SFR_SEG_FRES=>GWFSFRDAT(IGRID)%SFR_SEG_FRES
      GWFSFRDAT(IGRID)%SFR_SEG_FRES=>NULL()
      DEALLOCATE(SFR_SEG_FRES)
      SFR_SEG_FRES=>NULL()
      !
...
C     ------------------------------------------------------------------
      ...
      GWFSFRDAT(IGRID)%SFR_SEG_FRES   => SFR_SEG_FRES
