      MODULE OBSBASMODULE
         ...
         CHARACTER(:),SAVE,DIMENSION(:),POINTER,CONTIGUOUS::OBSNAM
       ...
       TYPE OBSBASTYPE
         ...
         CHARACTER(:),DIMENSION(:),POINTER,CONTIGUOUS::OBSNAM
...
C5------READ & PRINT ITEM 1 OF THE HOB INPUT FILE
     ...
     +                         HED="-- READING HOBS PACKAGE INPUT --")
...
C6------ALLOCATE ARRAY DATA.
      ALLOCATE(SKIP_OBS(NH), SOURCE=.FALSE.)
      ALLOCATE(NDER(5,NH))
      ALLOCATE(IOFF(NH))
      ALLOCATE(JOFF(NH))
      ALLOCATE(IHOBWET(NH))
      ALLOCATE(CHARACTER(OBSNAM_LENGTH):: OBSNAM(NH))
      ALLOCATE(H(NH))
      ALLOCATE(HOBS(NH))
      ALLOCATE(TOFF(NH))
      ALLOCATE(OTIME(NH))
...
C4------USER HAS DEFINED TOMULT CORRECTLY)
     ...
     +                              + DELT_TO_DAY(DBLE(TOFFMULT),ITMUNI)
          DATE = OBS_DATE%DATE
          WRITE(DYEAR,'(F13.7)') OBS_DATE%DYEAR
      ELSE
          DATE = 'NO_DATE' 
          DYEAR  = 'NO_DATE' 
      END IF
...
C         CALCULATE LENGTH OF FIRST TIME STEP IN CURRENT STRESS PERIOD
          DELT = SNGL(PERLEN(I))/REAL(NSTP(I))
