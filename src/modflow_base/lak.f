...
C
     ...
     +                        NODES, IUNIT, ITMUNI, LIST_OUT=>IOUT
...
C       argument lists are defined     
      ALLOCATE (NLAKES, NLAKESAR,THETA,LAKUNIT,NSFRLAK,NLKFLWTYP)
      ALLOCATE (IGSFLOWLAK)
      IGSFLOWLAK = 0
...
Cdep  initialize number of iterations and closure criteria to zero.
      ...
      SMALLTOL = 1.0d-4
      IF ( ITMUNI == 1 ) THEN
          SMALLTOL = SMALLTOL/86400.0D0
      ELSE IF ( ITMUNI == 2) THEN
          SMALLTOL = SMALLTOL/1440.0D0
      ELSE IF ( ITMUNI == 3) THEN    
          SMALLTOL = SMALLTOL/24.0D0
      ELSE IF ( ITMUNI == 5 ) THEN
          SMALLTOL = SMALLTOL*365.0D0
      END IF
      !
      lloc   = ONE
      IRDTAB = Z
      ...
      NPP    = Z
      MXVL   = Z
      CALL READ_TO_DATA(LINE,IN,IOUT,IOUT,NOSHIFT=NOFREE,
     +                          HED="-- READING LAK PACKAGE INPUT --")
! Check for alternate option to specify stage/vol/area tables.
   ...
   32  FORMAT(1X,I10,' Stage, volume and area relationship specified ',
     +                'based on an external tabular input file')
      CALL READ_TO_DATA(LINE,IN,IOUT,NOSHIFT=NOFREE)
      ELSE
...
Cdep  transient simulations when THETA is negative.
        IF(NOFREE) THEN
           ...
           !
           IF(I /= Z) CALL STOP_ERROR(LINE,IN,IOUT,MSG=
...
Cdep     LAKE Package.
        IF(THETA >= DZ .AND. NSSITR == Z) THEN
          NSSITR=100
          SSCNCR=1.0E-05
        ELSE IF(THETA < DZ)THEN 
          THETA=ABS(THETA) 
          IF(NOFREE) THEN 
...
!rsr, old data sets may not have SURFDEPTH, may need to trap this for some compilers 
     ...
     + MSG='LAK PACKAGE FAILED TO LOAD SURFDEPTH')
            ...
            IF ( IOS /= Z ) SURFDEPTH = DZ     !swm: This causes problems.  IOS is not defined.  Not sure what the correct fix is?
          END IF
        END IF
...
C    IF MXLKND TOO SMALL, ERROR MESSAGE WILL BE PRINTED.
      ...
      IF (NLAKES < ONE) THEN
          WRITE(IOUT,2)
          CLOSE(IN, IOSTAT=LLOC)
          IN = Z
          NLAKES = Z
      ELSE
        WRITE(IOUT,5) MXLKND,NLAKES
        IF (ILKCB >  Z) WRITE(IOUT,7) ILKCB
        IF (ILKCB <= Z) WRITE(IOUT,9)
...
Cdep      IF(ITRSS <= Z.AND.NSSITR == Z) NSSITR = 50
Cdep      IF(ITRSS <= Z.AND.SSCNCR.EQ.0.0) SSCNCR = 0.01
Cdep      IF(ITRSS == Z) WRITE(IOUT,23) NSSITR, SSCNCR
Cdep      IF(ITRSS.LT.0) WRITE(IOUT,24) NSSITR, SSCNCR
C-lfk  1     FORMAT(/1X,'LAK7 -- LAKE PACKAGE, VERSION 7, 2/06/2012',
1     FORMAT(/1X,'LAK7 -- LAKE PACKAGE, VERSION OWHM',
...
Cdep     2T, 0.5 IS CENTERED, AND 1.0 IS FULLY IMPLICIT.')
   22 FORMAT(/1X,'THETA = ',F6.3,/1X,'THETA IS THE TIME WEIGHTING ',
     ...
     *'OF LESS THAN 0.5 IS AUTOMATICALLY SET TO 0 AND LAKE STAGE IS ',
...
Cdep   ALLOCATE SPACE FOR FLOB ARRAY WHEN TRANSPORT ACTIVE.   
      IF (IUNITGWT /= Z .OR. IUNIT(49) /= Z) THEN
        MXLKAR = MXLKND
      ELSE
        MXLKAR = ONE
      END IF
...
!      END IF
      ...
      IF (NLAKES < ONE ) THEN
        CALL USTOP('nlakes dimension problem in lak7'//NUM2STR(nlakes))
      ENDIF
...
C     ------------------------------------------------------------------
     ...
     +                        LBOTM, BOTM, DELR, DELC, ISSFLG,IUNIT
      USE FILE_IO_INTERFACE, ONLY: MOVE_TO_DATA
C
      IMPLICIT NONE
C
...
!dep  added STGINIT as double precision
      ...
      LOGICAL:: NOFREE
      NOFREE = IFREFM == 0
      ANAME(1) = '           LAKE ID ARRAY'
      ANAME(2) = '  LAKEBED LEAKANCE ARRAY'
C
C     ------------------------------------------------------------------
...
! RGN 9/25/12 moved this to read lake bathymetry before stress period information.
 ...
 1399     FORMAT(//1X,'STAGE/VOLUME RELATION FOR LAKE',I3//6X,'STAGE',
     1        8X,'VOLUME',8X,'AREA'/)
          CALL MOVE_TO_DATA(LINE,iunitnum,IOUT)
          DO  INC=1,151
...
900   IF (IUNITBCF /= 0) THEN  ! rsr, moved if block from main
        CALL SGWF2LAK7BCF7RPS(LWRT)
      ELSE IF (IUNITLPF /= 0) THEN
        CALL SGWF2LAK7LPF7RPS(LWRT)
      ELSE IF (IUNITHUF /= 0) THEN
        CALL SGWF2LAK7HUF7RPS(LWRT)
      ELSE IF (IUNITUPW /= 0) THEN
        CALL SGWF2LAK7UPW1RPS(LWRT)
      ELSE
        CALL USTOP('LAK Package requires BCF, LPF, UPW, or HUF')
      END IF
      !
      IF (IUNITSFR /= 0) CALL SGWF2LAK7SFR7RPS()
      !
      IF(KKPER.EQ.1) THEN
        DO I=1,NLAKES
          STGOLD(I)  = STAGES(I)
