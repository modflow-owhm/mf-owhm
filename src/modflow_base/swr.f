...
C     + + + DATA + + +
      csolver(1) = "LU DECOMPOSITION USING CROUT'S ALGORITHM"
      csolver(2) = 'BICONJUGATE GRADIENT STABILIZED         '
      csolver(3) = 'GENERALIZED MINIMAL RESIDUAL            '
      !
      cmethod(1) = 'LEVEL-POOL ROUTING  '
      cmethod(2) = 'TILTED-POOL ROUTING '
      cmethod(3) = 'DIFFUSIVE-WAVE      '
      cmethod(4) = 'KINEMATIC-WAVE      '
      !
      cswght(0)  = 'CENTRAL             '
      cswght(1)  = 'UPSTREAM            '
      !
      ...
      cipc(0)    = 'NONE                '
      cipc(1)    = 'JACOBI              '
      cipc(2)    = 'INCOMPLETE LU       '
      cipc(3)    = 'MOD. INCOMPLETE LU  '
      cipc(4)    = 'INCOMPLETE LU THRES.'
      !
      cscale(0)  = 'NO SCALING          '
      cscale(1)  = 'SYMMETRIC SCALING   '
      cscale(2)  = 'L2-NORM SCALING     '
      !
      ctab(0)    = 'SPECIFIED TIMESTEP  '
      ctab(1)    = 'RAINFALL            '
      ctab(2)    = 'EVAPORATION         '
      ctab(3)    = 'LATERAL INFLOW      '
      ctab(4)    = 'STAGE               '
      ctab(5)    = 'STRUCTURE           '
      !
      cintp(1)   = 'NONE                '
...
C         PROBLEM DIMENSION AND OUTPUT CONTROL - INPUT ITEM 1
      ...
      CALL CHECK_CBC_GLOBAL_UNIT(ISWRCB, .FALSE.)
...
C     + + + FUNCTIONS + + +
      ...
      INTEGER         :: SSWR_CHKSTR
C     + + + DATA + + +
      clustor(1) = 'FULL JACOBIAN STORAGE (NSOLRG X NSOLRG)           '
      clustor(2) = 'BANDED JACOBIAN STORAGE (NSOLRG X JAC%NBW)        '
      !
      aname(1) = '        2D RAINFALL DATA'
      aname(2) = '     2D EVAPORATION DATA'
      aname(3) = '  2D LATERAL INFLOW DATA'
      aname(4) = '           2D STAGE DATA'
      !
      cstrbc(1) = 'NONE - CONNECTED REACHES      '
      cstrbc(2) = 'ZERO-DEPTH GRADIENT BOUNDARY  '
      cstrbc(3) = 'CRITICAL-DEPTH BOUNDARY       '
      cstrbc(4) = 'EXTERNAL STRUCTURE BOUNDARY   '
      cstrbc(5) = 'SFR PACKAGE INFLOW BOUNDARY   '
      cstrbc(6) = 'STAGE-DEPENDENT FLUX BOUNDARY '
...
!!!C     + + + CODE + + +
!!!        IF ( It.EQ.0 ) THEN
!!!          T1 = SECNDS(REAL(0.0,4))  !Non-Standard Fortran
!!!        ELSE
!!!          dt = SECNDS(REAL(T1,4))
!!!          Ts = Ts + DBLE(dt)
!!!        END IF
