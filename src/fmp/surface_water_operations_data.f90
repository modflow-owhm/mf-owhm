...
MODULE SWO_DATA_FMP_MODULE!, ONLY: SWO_DATA, INITIALIZE_SWO_DATA, FRAC_POOL2SPLIT, ACAP_STOR2AREA, ACAP_STOR2ELEV
  ...
  END SUBROUTINE
  !
  IMPURE ELEMENTAL SUBROUTINE DEALLOCATE_SWO_DATA(SWO)
  ...
  SUBROUTINE INITIALIZE_SWO_DATA( BL, SWO, LINE, FDIM, NSEG, NSTRM, SEG_NSTRM, FASTFORWARD, MXITER )
    ...
    DO WHILE (IERR == Z)
      ...
      CASE ("HEAD_BOUNDARY_CELLS","HEAD_BOUNDARY_CELL")
                        ...
                        DEALLOCATE(FILES)
                        !
      CASE ("RESERVOIR_PRIMARY_REREGULATION_SPLIT", "REREGULATION_SPLIT", "RESERVOIR_SPLIT_FRACTIONS")
                        WRITE(BL%IOUT,'(A)') 'RESERVOIR_PRIMARY_REREGULATION_SPLIT (REREGULATION_SPLIT or RESERVOIR_SPLIT_FRACTIONS) KEYWORD FOUND. NOW LOADING '//NUM2STR(SWO%NRES_TOTTOT)//' RESERVOIR FRACTIONS. USE THE WORD SKIP OR NULL FOR RESERVOIRS THAT ARE NOT PART OF A PRIMARY/REREGULATION SYSTEM.'
                        I=Z  !TEMP VARIABLE USED FOR UNIT NUMBER...MUST BE ZERO TO TELL ULOAD TO QUERY FILE UNIT
                        !
                        ALLOCATE(FILES(SWO%NRES_TOTTOT))
                        HAS_SPLIT_FRAC = TRUE
                        !
                        CALL ULOAD(FILES, LLOC, LINE, BL%IOUT, BL%IU, I, SCRATCH=BL%SCRATCH, MSG='ERROR SOMETIME AFTER SWO KEYWORD RESERVOIR_PRIMARY_REREGULATION_SPLIT')
                        !
                        K=Z
                        DO I = ONE, SWO%NPROJ
                              DO J = ONE, SWO%NRES_TOT(I)
...
!!                        !
    ...
    IF(HAS_SPLITNAM    .AND. .NOT. HAS_SPLIT_FRAC        ) CALL WARN_MSG1%ADD('FAILED TO IDENTIFY/LOCATE WITHIN BLOCK "RESERVOIR_PRIMARY_REREGULATION_SPLIT" KEYWORD,'//NL//'WHICH IS REQUIRED WHEN THERE ARE SPLIT RESERVOIRS THAT ARE TREATED AS ONE LUMPED POOL STORAGE'//NL//'(viz. THERE WAS A PROJECT_RESERVOIR_COUNT < 0). [NOTE THAT "SPLIT_FRACTIONS" KEYWORD WORKS AS WELL.]'//BLN)
    !
...
!
      ...
      DO ISPT = 1, SPTCOUNT
        ...
        IF(SPTSEG(ISPT)%NBRANCH > Z) THEN
          SPTSEG(ISPT)%BrcDIVERSION(:) = DZ
          SPTSEG(ISPT)%BrcDELIVERY(:)  = DZ
          SPTSEG(ISPT)%BrcDELIVEFF(:)  = UNO
        END IF
      END DO
      !
