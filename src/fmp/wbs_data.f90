...
MODULE WBS_DATA_FMP_MODULE
  ...
  SUBROUTINE INITIALIZE_WBS_DATA( BL, WBS, LINE, FDIM )  
    ...
    DO WHILE (.NOT. EOF)
      ...
      CASE ("PRORATE","PRORATE_DEFICIENCY")
                        ...
                        END SELECT
      !
      CASE ("IRRIGATION_EFFICIENCY", "EFFICIENCY", "EFF", "OFE")
                        WRITE(BL%IOUT,'(A)') '   IRRIGATION_EFFICIENCY (EFF)      KEYWORD FOUND. NOW LOADING LIST/ARRAY KEY AND STATIC/TRANSIENT KEYWORDS AND FIRST SET OF EFFICIENCIES.'
                        IF(FDIM%NIRRG > Z) THEN
                            CALL WBS%EFF_TFR%INIT('EFF', LLOC, LINE, BL%IOUT, BL%IU, WBS%NFARM, FDIM%NIRRG, FDIM%NROW, FDIM%NCOL, WBS%NFARM, BYFARM, NCROP, BYCROP, FDIM%NIRRG, 'BYIRRIGATE', SCRATCH=BL%SCRATCH, LISTARRAY=TRUE)  !, MSG='ALSO PLEASE DOUBLE CHECK THAT NIRRIGATE IN THE GLOBAL DIMENSION BLOCK IS SET UP CORRECTLY. IRRIGATION_EFFICIENCY EXPECTS TO READ AN ID COLUMN AND THEN NIRRIGATE COLUMNS OF EFECIENCIES.'
                        ELSE
                            CALL WBS%EFF_TFR%INIT('EFF', DZ, BL%IOUT, BL%IU, WBS%NFARM, NIRRG, FDIM%NROW, FDIM%NCOL, LISTARRAY=TRUE)
                            CALL WARNING_MESSAGE(OUTPUT=BL%IOUT,MSG='FMP WBS BLOCK: FOUND KEYWORD "IRRIGATION_EFFICIENCY" WHEN NIRRIGATE = 0. THIS LINE WILL BE IGNORED AND THE IRRIGATION_EFFICIENCY SET TO 0.0 FOR ALL WBS (FARMS).',CMD_PRINT=TRUE)
                        END IF
                        !
      CASE ("IRRIGATION_EFFICIENCY_IMPROVEMENT","EFFICIENCY_IMPROVEMENT","EFF_IMPROVE")
                        WRITE(BL%IOUT,'(A)') '   IRRIGATION_EFFICIENCY_IMPROVEMENT KEYWORD FOUND. NOW LOADING LIST/ARRAY KEY AND STATIC/TRANSIENT KEYWORDS AND FIRST SET OF IRRIGATION_EFFICIENCY IMPROVEMENT FLAGS (0 OR 1).'
                        IF(FDIM%NIRRG > Z) THEN
                        ...
                        ELSE
                            ...
                            CALL WARNING_MESSAGE(OUTPUT=BL%IOUT,MSG='FMP WBS BLOCK: FOUND KEYWORD "IRRIGATION_EFFICIENCY_IMPROVEMENT" WHEN NIRRIGATE = 0. THIS LINE WILL BE IGNORED AND THE IMPROVEMENT FLAG SET TO 0 FOR ALL WBS (FARMS).',CMD_PRINT=TRUE)
                        END IF
                        !
      CASE ("ADDED_DEMAND_RUNOFF_SPLIT", "ADRS")
                        ...
                        ELSE
                            ...
                            CALL WARNING_MESSAGE(OUTPUT=BL%IOUT,MSG='FMP WBS BLOCK: FOUND KEYWORD "ADDED_DEMAND_RUNOFF_SPLIT" WHEN NIRRIGATE = 0. THIS LINE WILL BE IGNORED AND THE IRRIGATION_EFFICIENCY SET TO 0.0 FOR ALL WBS (FARMS).',CMD_PRINT=TRUE)
                        END IF
                        !
      ...
      !!!                      CALL WARNING_MESSAGE(OUTPUT=BL%IOUT,MSG='FMP WBS BLOCK: FOUND KEYWORD "" WHEN NIRRIGATE = 0. THIS LINE WILL BE IGNORED AND THE IRRIGATION_EFFICIENCY SET TO 0.0 FOR ALL WBS (FARMS).',CMD_PRINT=TRUE)
      !!!                  END IF
      !
      !CASE ("FALLOW_RANK")
    ...
    IF(.NOT. WBS%EFF_TFR%INUSE) THEN
        IF(FDIM%NIRRG>Z) CALL STOP_ERROR(INFILE=BL%IU, OUTPUT=BL%IOUT,MSG='FAILED TO LOCATE KEYWORD "IRRIGATION_EFFICIENCY" WITHIN FMP WBS BLOCK.'//BLN//'IT IS A REQUIRED INPUT IF YOU HAVE NIRRIGATE > 0.'//BLN//'IF YOU DO NOT HAVE IRRIGATION, YOU MUST SET NIRRIGATE=0 IN THE GLOBAL DIMENSION BLOCK.'//BLN//'PLEASE DOUBLE CHECK BLOCK SET UP.') 
        !
        CALL WBS%EFF_TFR%INIT('EFF', DZ, BL%IOUT, BL%IU, WBS%NFARM, NIRRG, FDIM%NROW, FDIM%NCOL, LISTARRAY=TRUE)
    END IF
    !
    ! IF(.NOT. WBS%ADRF_TFR%INUSE .AND. FDIM%NIRRG>Z) THEN ! Moved to crop_data.f90 routine setup_next_stress_period
    !     !CALL WBS%ADRF_TFR%INIT('ADRS', TENTH, BL%IOUT, BL%IU, WBS%NFARM, NIRRG, FDIM%NROW, FDIM%NCOL, LISTARRAY=TRUE) -Not needed anymore cuz its auto-allocated at 0.1
    ...
    IF(.NOT. WBS%EFF_IMPROVE_TFR%INUSE) THEN
        ...
        !IF(FDIM%NIRRG > Z .AND. (ANY(WBS%DEFICIENCY%LIST.NE.Z).OR.WBS%DEFICIENCY%TRANSIENT)) CALL WARN_MSG%ADD('FAILED TO LOCATE KEYWORD "IRRIGATION_EFFICIENCY_IMPROVEMENT" WITHIN FMP WBS BLOCK.'//NL//'IT WILL BE SET TO 0, WHICH INDICATES EFFICIENCIES ARE NOT IMPROVED UNDER DEFICIT IRRIGATION SCENARIOS.'//NL//'THIS KEEPS THE IRRIGATION_EFFICIENCY THE SAME AS IT IS SPECIFIED BY INPUT AND IS THE RECOMMEND OPTION.'//BLN)
    END IF
    !
  ...
  SUBROUTINE SETUP_NO_WBS_DATA( WBS, FDIM, IN_FMP, IOUT )  
    ...
    IF(FDIM%NIRRG>Z) CALL STOP_ERROR(INFILE=IN_FMP, OUTPUT=IOUT,                                             &
                                    ...
                                    'AND SPECIFY THE "IRRIGATION_EFFICIENCY" WITHIN IT.'//NL//                          &
