...
MODULE WELL_DATA_FMP_MODULE
  ...
  USE STRINGS,                          ONLY: GET_INTEGER, GET_NUMBER, GET_WORD, UPPER
...
!  END SUBROUTINE
  ...
  END SUBROUTINE
  !
  IMPURE ELEMENTAL SUBROUTINE FINAL_DEALLOCATE_FWELL_DIMENSIONS(FWEL)
  ...
  IMPURE ELEMENTAL SUBROUTINE FWEL_READ_N_PREPARE(FWEL, SP)  !IMPURE FORCES CALLS TO BE IN ORDER OF FARM ID (eg 1, 2, 3, ... NFARM)
    ...
    CASE(3) ! LINEFEED CAPACITY CHANGING
            ...
            READ(FWEL%FEED%IU,*,IOSTAT=IERR) FWEL%Qini
                                            IF(IERR.NE.Z) CALL FILE_IO_ERROR( IERR, FWEL%FEED%IU, LINE=LINE, OUTPUT=FWEL%IOUT, MSG= "FMP SUPPLY_WELL LINEFEED ERROR WHILE READING CURRENT STRESS PERIOD'S FEED")
            !IF (LFID) THEN
  ...
  SUBROUTINE INITIALIZE_FMP_WELL_DATA( BL, FWEL, LN )  !=ASSUMES FWEL IS OF SIZE NFARMS  ---Called from FMP main to load FWEL 
    ...
    !
    WRITE(BL%IOUT,'(/A/)') 'SUPPLY_WELL BLOCK FOUND AND NOW LOADING PROPERTIES'
    !
    INSTYLE = Z
    INPUT_TYPE = Z
    ...
    !
    CALL BL%START()
    IF( BL%LINE==ERROR ) CALL STOP_ERROR(INFILE=BL%IU, OUTPUT=BL%IOUT,MSG='UNFORTUNATELY UNKNOWN FMP SUPPLY_WELL BLOCK ERROR. PLEASE DOUBLE CHECK BLOCK SET UP.')
    !
    ...
    DO N=ONE, BL%NLINE
      !
      IF(  BL%LINE==ERROR ) EXIT !--THIS SHOULD NOT HAPPEN!!! CALL STOP_ERROR(INFILE=BL%IU, OUTPUT=BL%IOUT, MSG='UNFORTUNATELY UNKNOWN FMP SUPPLY_WELL BLOCK ERROR. THIS MAY HAVE HAPPENED BECAUES IT FAILED TO LOAD THE LAST KEYWORD WHICH IS ANY OF THE FOLLOWING: "TIME FRAME" OR "LINEFEED WBS" OR "LINEFEED CAPACITY". PLEASE DOUBLE CHECK BLOCK SET UP.')
      !
      LLOC=ONE
      ...
      CASE ("SFAC")
                        ...
                        !
                        !IF(.NOT. ALLOCATED(LN)) ALLOCATE(CHARACTER(768):: LN)   !DUBLICATE VARIABLE MADE IN CASE BL%LIST%LN IS VERY SMALL
      ...
      CASE ("INPUT_OPTION")
                        ...
                        CASE ("DEPTH")
                                           INSTYLE = INSTYLE + 20
                        CASE DEFAULT;      CALL STOP_ERROR(BL%LINE, BL%IU, BL%IOUT, 'FMP SUPPLY_WELL BLOCK KEYWORD ERROR. IDENTIFIED KEYWORD "INPUT" WHICH INDICATES NEXT WORD SHOULD BE "XY" OR "ELEVATION"  OR "DEPTH" TO INDICATE HOW INPUT IS INTERPRETED.')
                        END SELECT
                        !
                        IF( ALL( INSTYLE .NE. [0, 1, 10, 11, 20, 21] )) CALL STOP_ERROR(BL%LINE,BL%IU,BL%IOUT,MSG='FMP SUPPLY_WELL BLOCK KEYWORD ERROR. INPUT STYLE DECLARATION CONTAINS AN ERROR. EITHER OU SPECIFIED XY TWICE OR CONTAINED ELEVATION AND DEPTH AS KEYWORDS.')
      CASE ("SMOOTH", "SMOOTHING")
                        ...
                        CASE ("BY_THICK", "BYTHICK")
                                           FWEL%SMOOTH = TWO
                        CASE DEFAULT;      CALL STOP_ERROR(BL%LINE,BL%IU,BL%IOUT,MSG='FMP SUPPLY_WELL BLOCK KEYWORD ERROR. IDENTIFIED KEYWORD "SMOOTH" WHICH INDICATES NEXT WORD SHOULD BE "BY_FRACTION" OR "BY_THICK" TO INDICATE THE TYPE OF PRORATION.')
                        END SELECT
                        !
                        !IF(.NOT. ALLOCATED(LN)) ALLOCATE(CHARACTER(768):: LN)   !DUBLICATE VARIABLE MADE IN CASE BL%LIST%LN IS VERY SMALL
                        ...
                        ELSE
                                                            CALL STOP_ERROR(BL%LINE,BL%IU,BL%IOUT,MSG='FMP SUPPLY_WELL BLOCK UNKNOWN ERROR. FAILED TO LOAD SMOOTH FACTOR.')
                        END IF
      CASE ("PRORATE_DEMAND", "PRORATE")
                                         ...
                                         IF(IERR==Z                           .OR.  &
                                            ...
                                            BL%LINE(ISTART:ISTOP) == 'ALL'       ) THEN  !PROCESS INFORMATION WITH SFAC DATA TYPE
                                               ...
                                               ELSEIF(PRORATE_Q_FACTOR%HAS_ALL) THEN
                                                      ...
                                                      END IF
                                               ELSE
                                                          CALL STOP_ERROR(BL%LINE,BL%IU,BL%IOUT,MSG='FMP SUPPLY_WELL BLOCK UNKNOWN ERROR. FAILED TO LOAD PRORATE BYQMAX.')
                                               END IF
                                         END IF
                                         !
                        ...
                        CASE DEFAULT;      CALL STOP_ERROR(BL%LINE,BL%IU,BL%IOUT,MSG='FMP SUPPLY_WELL BLOCK KEYWORD ERROR. IDENTIFIED KEYWORD "PRORATE" WHICH INDICATES NEXT WORD SHOULD BE "BYQMAX" OR "BYAVERAGE" TO INDICATE THE TYPE OF PRORATION.')
                        END SELECT
      CASE ("PRINT")
                        IF(INPUT_TYPE > Z) INPUT_TYPE = -INPUT_TYPE  !FLIP SIGN TO STOP ADDING LINES
                        !
                        ...
                        CASE ("SMOOTHING")
                                          CALL FWEL(ONE)%OUT_SMOOTH%OPEN(BL%LINE,LLOC,BL%IOUT,BL%IU,BINARY=BINARY,SPLITMAXCOUNT=11)
                                                                                                   !
                                                                                                   IF(BINARY) CALL WARNING_MESSAGE(OUTPUT=BL%IOUT,MSG='FMP SUPPLY_WELL: FARM WELL "PRINT SMOOTHING" CONTAINED KEYWORD "BINARY" BUT IT DOES NOT SUPPORT BINARY WRITING, SO INSTEAD A TEXT FILE WILL BE WRITTEN. THIS MAY CAUSE A CRASH IF YOU OPENED THE FILE UNIT WITH DATA(BINARY).',INLINE=TRUE) 
                        CASE ("INPUT")
                                          CALL FWEL(ONE)%OUT_INPUT%OPEN(BL%LINE,LLOC,BL%IOUT,BL%IU,SPLITMAXCOUNT=11)
                                                                                                   !
                                                                                                   IF(BINARY) CALL WARNING_MESSAGE(OUTPUT=BL%IOUT,MSG='FMP SUPPLY_WELL: FARM WELL "PRINT INPUT" CONTAINED KEYWORD "BINARY" BUT IT DOES NOT SUPPORT BINARY WRITING, SO INSTEAD A TEXT FILE WILL BE WRITTEN. THIS MAY CAUSE A CRASH IF YOU OPENED THE FILE UNIT WITH DATA(BINARY).',INLINE=TRUE) 
                                                                                                   !
                        CASE DEFAULT;     CALL STOP_ERROR(BL%LINE,BL%IU,BL%IOUT,'FMP SUPPLY_WELL BLOCK KEYWORD ERROR. IDENTIFIED KEYWORD "PRINT", BUT THE NEXT WORD WAS NOT IDENTIFIED. WORDS EXPECTED ARE: "BYFARM", "BYWELL", "SMOOTHING", "INPUT"')
                        END SELECT
      CASE ("QMAXRESET")
                        IF(INPUT_TYPE > Z) INPUT_TYPE = -INPUT_TYPE  !FLIP SIGN TO STOP ADDING LINES
                        !
      ...
      CASE ("LINEFEED", "FEEDFILE", "FEEDFILES")
                        ...
                        CASE ("CAPACITY")
                                         INPUT_TYPE = THREE
                                         !
                        CASE DEFAULT;    CALL STOP_ERROR(BL%LINE,BL%IU,BL%IOUT,'FMP SUPPLY_WELL BLOCK KEYWORD ERROR. IDENTIFIED KEYWORD "'//BL%LINE(ISTART:ISTOP)//'", BUT THE NEXT WORD WAS NOT IDENTIFIED. WORDS EXPECTED ARE: "WBS" OR "CAPACITY"')
                        END SELECT
                        !
                        CALL INWEL%ADD_LINE(BL%LINE)
                        !
      ...
      CASE DEFAULT
                 IF(INPUT_TYPE > Z) THEN
                     CALL INWEL%ADD_LINE(BL%LINE)
                 ELSE
                     CALL WARNING_MESSAGE(OUTPUT=BL%IOUT,MSG='FMP SUPPLY_WELL BLOCK FOUND UNKNOWN KEYWORD "'//BL%LINE(ISTART:ISTOP)//'" ***IT WILL BE IGNORED***',INLINE=TRUE,CMD_PRINT=TRUE)
                 END IF
      END SELECT
      !
      CALL BL%NEXT()
    ...
    CASE(THREE)
               ...
               END DO
    CASE DEFAULT
               CALL STOP_ERROR(BLNK,BL%IU,BL%IOUT,'FMP SUPPLY_WELL BLOCK FAILED TO LOCATE ONE OF THE FOLLOWING KEYWORDS:'//NL//'TIME  FRAME'//NL//'LINEFEED WBS'//NL//'LINEFEED CAPACITY'//NL//'ONE OF THESE THREE KEYWORDS MUST BE PRESENT TO CONTINUE A SIMULATION WITH FARM WELLS')
    END SELECT
    ...
    IF(MXDIM>Z) THEN
        ...
        IF (FWEL(ONE)%LOAD_TYPE == TWO ) THEN
          DO FD=ONE, SIZE(FWEL(ONE)%FID_FEED)
            I=ONE
            DO K=ONE, SIZE(FWEL(ONE)%FID_FEED(FD)%MASTER_LOC)
  ...
  SUBROUTINE FWEL_PROPERTY_READ(BL,WELLID,IL,IR,IC,NAUX,AUX,INSTYLE,NF,Q,SPSTART,SPEND)
    ...
    INTEGER,ALLOCATABLE, DIMENSION(:), INTENT(INOUT):: AUX
    ...
    ELSE
        IF(INSTYLE > ONE) THEN
                     ...
                     IF(IERR.NE.Z .OR. BL%LIST%LN(ISTART:ISTOP)=='') CALL FILE_IO_ERROR(IERR,BL%IU,LINE=BL%LIST%LN,OUTPUT=BL%IOUT,MSG='FMP SUPPLY_WELL BLOCK FAILED TO CONVERT "ELEVATION" OR "DEPTH" TO A REAL NUMBER.')
                     ELEV = ELEV + TOL*ELEV
        ...
        ELSE
                     ...
                     IF(IERR.NE.Z .OR. BL%LIST%LN(ISTART:ISTOP)=='') CALL FILE_IO_ERROR(IERR,BL%IU,LINE=BL%LIST%LN,OUTPUT=BL%IOUT,MSG='FMP SUPPLY_WELL BLOCK FAILED TO CONVERT "LAYER" TO AN INTEGER.')
        END IF
    END IF
    !
    ...
    IF    ( IL < Z ) THEN
                        ...
                        DO I = ONE, NLAY
                                        ...
                                        END IF
                        END DO
                        IF(IL < Z) CALL STOP_ERROR(BL%LIST%LN,BL%IU,BL%IOUT,'FMP SUPPLY_WELL BLOCK FAILED TO FIND A LAYER FOR THE SPECIFIED OR CALCULATED "ELEVATION" OF '//NUM2STR(ELEV))
    END IF
    !
    IF( IL /= Z) THEN
    ...
    IF(PRESENT(SPSTART)) THEN
                       ...
                       ELSE
                           ...
                           IF    (SPSTART == -1) THEN
                                  ...
                                  IF(IERR.NE.Z .OR. BL%LIST%LN(ISTART:ISTOP)=='') CALL FILE_IO_ERROR(IERR,BL%IU,LINE=BL%LIST%LN,OUTPUT=BL%IOUT,MSG='FMP SUPPLY_WELL BLOCK READ FAILED TO IDENTIFY STARTING STRESS PERIOD OR STARTING DATE (MAKE SURE DATES ARE OF THE FORM MM/DD/YYYY)')
                                  !
                           ELSEIF(SPSTART == -2) THEN
                                    CALL STOP_ERROR(BL%LIST%LN,BL%IU,BL%IOUT,'FMP SUPPLY_WELL BLOCK LIST READ SPECIFIED A STARTING DATE BUT THE DIS PACKAGE KEYWORD STARTDATE IS MISSING (OneWater IS NOT DATE AWARE WITHOUT A STARTING DATE).')
                           END IF
                       END IF
                       !
                       CALL PARSE_WORD_UP(BL%LIST%LN,LLOC,ISTART,ISTOP)
                       ...
                       ELSE
                           ...
                           IF    (SPEND == -1) THEN
                                  ...
                                  IF(IERR.NE.Z .OR. BL%LIST%LN(ISTART:ISTOP)=='') CALL FILE_IO_ERROR(IERR,BL%IU,LINE=BL%LIST%LN,OUTPUT=BL%IOUT,MSG='FMP SUPPLY_WELL BLOCK READ FAILED TO IDENTIFY ENDING STRESS PERIOD OR ENDING DATE (MAKE SURE DATES ARE OF THE FORM MM/DD/YYYY)')
                                  !
                           ELSEIF(SPEND == -2) THEN
                                    CALL STOP_ERROR(BL%LIST%LN, BL%IU, BL%IOUT, 'FMP SUPPLY_WELL BLOCK LIST READ SPECIFIED A ENDING DATE BUT THE DIS PACKAGE KEYWORD STARTDATE IS MISSING (OneWater IS NOT DATE AWARE WITHOUT A STARTING DATE).')
                           END IF
                       END IF
                       !
                       IF( SPSTART > NPER  ) SPEND = SPSTART+ONE  !CASE WHEN DATESTART IS BEYOND SIMULATION, AND DATE END USES NAN
                       IF( SPSTART > SPEND ) CALL STOP_ERROR(BL%LIST%LN, BL%IU, BL%IOUT, 'FMP SUPPLY_WELL BLOCK LIST READ SPECIFIED A STARTING STRESS PERIOD/DATE THAT IS GREATER THAN THE ENDING DATE.')
    END IF
    !
    IF(NAUX > Z) THEN
               NAUX_STR = NUM2STR(NAUX)
               DO I=ONE, NAUX
                   CALL GET_INTEGER(BL%LIST%LN,LLOC,ISTART,ISTOP,BL%IOUT,BL%IU,AUX(I),MSG='FMP SUPPLY_WELL BLOCK LIST READ FAILED FAILED TO LOAD AUXILIARY FLAG (0 or 1). THERE SHOULD BE A TOTAL OF '//NAUX_STR//' AUX VARIABLES READ IN FOR EACH WELL [abc].') 
               END DO
    END IF
    !
  END SUBROUTINE
...
!  END SUBROUTINE
  ...
  SUBROUTINE FWEL_LOAD_CAPACITY_LINEFEED(BL,FWEL,INSTYLE)
    ...
    CALL BL%LIST%NEXT()
    CALL BL%LIST%SET_LN()
    IF(  BL%LIST%LN==ERROR ) CALL STOP_ERROR(INFILE=BL%IU, OUTPUT=BL%IOUT, MSG='UNFORTUNATELY UNKNOWN FMP SUPPLY_WELL BLOCK ERROR. PLEASE DOUBLE CHECK BLOCK SET UP.')
    !!
    !LLOC=ONE 
    ...
    DO I=BL%LIST%GETPOS(), BL%NLINE
        ...
        ELSE
            ...
            IF(FWEL(NF)%FEED%ERROR) CALL STOP_ERROR(BL%LIST%LN, BL%IU, BL%IOUT, 'FMP SUPPLY_WELL BLOCK FAILED TO OPEN WBS FEED FILE AFTER "LINEFEED CAPACITY" KEYWORD.'//NEW_LINE('')//'THIS COULD BE BECAUSE EITHER THE UNIT NUMBER COULD NOT BE IDENTIFIED (e.g. "EXTERNAL UNIT")'//NEW_LINE('')//'OR THE PATH IS BAD (e.g. "OPEN/CLOSE FILE")') 
            IF(FWEL(NF)%FEED%IU==Z) CALL STOP_ERROR(BL%LIST%LN, BL%IU, BL%IOUT, 'FMP SUPPLY_WELL BLOCK DOES NOT ALLOW FOR THE KEYWORD "CONSTANT" OR "INTERNAL" AFTER THE KEYWORD "LINEFEED CAPACITY". LINEFEED INPUT MUST BE SPECIFIED IN A SPEARATE FILE.') 
        END IF
        !
        CALL BL%LIST%NEXT()
        CALL BL%LIST%SET_LN()
    ...
    DO NF=1, NFARM
        ...
        IF(IU .NE. Z) THEN
                          ...
                          !
                          IF(NO_END_ERROR)  CALL STOP_ERROR(INFILE=IU, OUTPUT=BL%IOUT, MSG='FMP SUPPLY_WELL BLOCK "LINEFEED CAPACITY" ERROR WHEN LOADING FROM LINEFEED FILE DESCRIBED ABOVE.'//NL//'THE STATIC/STRUCTAL INPUT MUST BE SEPARATATED FROM THE TRANSIENT/CAPACITY INPUT WITH THE KEYWORD "TEMPROAL".'//NL//'FAILED TO LOCATE THE KEYWORD TEMPORAL AS THE FIRST WORD ON A LINE.')
                          !
                          CALL BL%LIST%START()
                          CALL BL%LIST%SET_LN()
                          IF(  BL%LIST%LN==ERROR ) CALL STOP_ERROR(INFILE=BL%IU, OUTPUT=BL%IOUT, MSG='UNFORTUNATELY UNKNOWN FMP SUPPLY_WELL BLOCK ERROR. POSSIBLE ERROR READING FIRST LINE OF LINEFEED FILE FOR FARM '//NUM2STR(NF))
                          !
                          N = BL%NLINE
                          ...
                          DO I=1, N
                              ...
                              CALL BL%LIST%NEXT()
                              CALL BL%LIST%SET_LN()
                              !IF(  BL%LIST%LN==ERROR .AND. I.NE.N) CALL FILE_IO_ERROR(Z,BL%IU, OUTPUT=BL%IOUT,MSG='UNFORTUNATELY UNKNOWN FMP SUPPLY_WELL BLOCK ERROR.  POSSIBLE CRASH HAPPENED WHEN LOADING WELL PROPERTIES IN A FEEDFILE WITH THE PERVIOUS WELL BEING:'//TRIM(WELLID)')
                          END DO
                          !
  ...
  SUBROUTINE FWEL_LOAD_FID_LINEFEED(BL,FWEL,INSTYLE)
    ...
    !!
    !CALL BL%LIST%SET_LN()
    !IF(  BL%LIST%LN==ERROR ) CALL STOP_ERROR(INFILE=BL%IU, OUTPUT=BL%IOUT, MSG='UNFORTUNATELY UNKNOWN FMP SUPPLY_WELL BLOCK ERROR. POSSIBLE ERROR TRYING TO READ IN PART OF BLOCK THAT PERTAINS TO THE ACTUAL FEEDFILE NAMES.')
    !
    !GET FEED FILE COUNT
    ...
    DO I=ONE, NFEED
        ...
        IF (FWEL(ONE)%FID_FEED(I)%FL%ERROR) CALL STOP_ERROR(BL%LIST%LN, BL%IU, BL%IOUT, 'FMP SUPPLY_WELL BLOCK FAILED TO OPEN WBS FEED FILE AFTER "LINEFEED WBS" KEYWORD.'//NEW_LINE('')//'THIS COULD BE BECAUSE EITHER THE UNIT NUMBER COULD NOT BE IDENTIFIED (e.g. "EXTERNAL UNIT")'//NEW_LINE('')//'OR THE PATH IS BAD (e.g. "OPEN/CLOSE FILE")') 
        IF (FWEL(ONE)%FID_FEED(I)%FL%IS_CONSTANT) CALL STOP_ERROR(BL%LIST%LN, BL%IU, BL%IOUT, 'FMP SUPPLY_WELL BLOCK DOES NOT ALLOW FOR THE KEYWORD "CONSTANT" AFTER THE KEYWORD "LINEFEED WBS". LINEFEED INPUT MUST BE SPECIFIED IN A SPEARATE FILE.') 
        IF (FWEL(ONE)%FID_FEED(I)%FL%IS_INTERNAL) CALL STOP_ERROR(BL%LIST%LN, BL%IU, BL%IOUT, 'FMP SUPPLY_WELL BLOCK DOES NOT ALLOW FOR THE KEYWORD "INTERNAL" AFTER THE KEYWORD "LINEFEED WBS". LINEFEED INPUT MUST BE SPECIFIED IN A SPEARATE FILE.') 
        !
        CALL BL%LIST%NEXT()
        CALL BL%LIST%SET_LN()
    END DO
    ...
    DO FD=1, NFEED
        ...
        IF(IU .NE. Z) THEN
                          CALL BL%INNER('TEMPORAL',IU,BL%IOUT,END_NOT_FOUND=NO_END_ERROR)  !LOAD LINEFEED DEFINITION LINES
                          !
                          IF(NO_END_ERROR)  CALL STOP_ERROR(INFILE=IU, OUTPUT=BL%IOUT, MSG='FMP SUPPLY_WELL BLOCK "LINEFEED WBS" OR "LINEFEED FARM" ERROR WHEN LOADING FROM LINEFEED FILE DESCRIBED ABOVE.'//NL//'THE STATIC/STRUCTAL INPUT MUST BE SEPARATATED FROM THE TRANSIENT/CAPACITY INPUT WITH THE KEYWORD "TEMPROAL".'//NL//'FAILED TO LOCATE THE KEYWORD TEMPORAL AS THE FIRST WORD ON A LINE.')
                          !
                          !
                          CALL BL%LIST%START()
                          CALL BL%LIST%SET_LN()
                          IF(  BL%LIST%LN==ERROR ) CALL STOP_ERROR(INFILE=BL%IU, OUTPUT=BL%IOUT,MSG='UNFORTUNATELY UNKNOWN FMP SUPPLY_WELL BLOCK ERROR. POSSIBLE ERROR READING THE '//NUM2STR(FD)//' LINEFEED FILE')
                          !
                          N = BL%NLINE
  ...
  SUBROUTINE FWEL_LOAD_LIST(BL,FWEL,INSTYLE)
    ...
    IF(  BL%LIST%LN==ERROR ) CALL STOP_ERROR(INFILE=BL%IU, OUTPUT=BL%IOUT,MSG='UNFORTUNATELY UNKNOWN FMP SUPPLY_WELL BLOCK ERROR. PLEASE DOUBLE CHECK BLOCK SET UP.')
    !
    !First check if well input is directly after keyword
    LLOC = ONE
    CALL PARSE_WORD_UP(BL%LIST%LN,LLOC,ISTART,ISTOP)                                     ! WELID
    CALL GET_INTEGER(BL%LIST%LN,LLOC,ISTART,ISTOP,Z,Z,I,HAS_ERROR=CHECK)                 ! NF
    IF(.NOT. CHECK) CALL GET_INTEGER(BL%LIST%LN,LLOC,ISTART,ISTOP,Z,Z,I,HAS_ERROR=CHECK) ! LAY
    IF(.NOT. CHECK) CALL GET_INTEGER(BL%LIST%LN,LLOC,ISTART,ISTOP,Z,Z,I,HAS_ERROR=CHECK) ! ROW
    IF(.NOT. CHECK) CALL GET_INTEGER(BL%LIST%LN,LLOC,ISTART,ISTOP,Z,Z,I,HAS_ERROR=CHECK) ! COL
    IF(.NOT. CHECK) CALL GET_NUMBER (BL%LIST%LN,LLOC,ISTART,ISTOP,Z,Z,Q,HAS_ERROR=CHECK) ! Qcap 
    !
    IF(CHECK) THEN
        LLOC = ONE
    ...
    END IF
    !
    IF (.not. CHK_FL%IS_INTERNAL) THEN
        NFILE = BL%NLINE - BL%LIST%GETPOS() + ONE
        CALL BL%LIST%NEXT()
    ELSE
        NFILE = ONE
        IF(CHECK) CALL BL%LIST%NEXT()  
        CALL BL%MAKE_SCRATCH_FILE(FROM_START=FALSE)  !WRITE REMAINING PORTION TO SCRATCH FILE
        CALL BL%GOTO_END()
        !
        Q =CHK_FL%SCALE
        !
    ...
    IF(NFILE > ONE) THEN
      ...
      DO J=BL%LIST%GETPOS(), BL%NLINE
          !
          CALL BL%LIST%SET_LN()
          IF(  BL%LIST%LN==ERROR ) CALL STOP_ERROR(INFILE=BL%IU, OUTPUT=BL%IOUT, MSG='UNFORTUNATELY UNKNOWN FMP SUPPLY_WELL BLOCK ERROR. POSSIBLE ERROR READING FIRST LINE OF LIST LOAD.')
          !
          I = I + ONE
          LLOC  = ONE
    ...
    END DO; END DO
    IF (ERR .NE. '') CALL STOP_ERROR(OUTPUT=BL%IOUT, MSG='FMP SUPPLY_WELL CONTAINED WELLS WITH ENDING DATES/STRESS PERIODS THAT ARE BEFORE THE STARTING DATE/STRESS PERIOD. THE FOLLOWING WELL NAMES ARE THE WELLS THAT HAD THIS PROBLEM WITH THEIR STARTING AND ENDING STRESS PERIODS.'//NL//ERR//NL)
    !
    CALL BL%LIST%DESTROY()
    !
...
!  END SUBROUTINE
  ...
  SUBROUTINE FWEL_FID_LINEFEED_MNW2_BUILD(FWEL)
    ...
    NFEED = SIZE(FWEL%FID_FEED)
    !
    IF(ANY(FWEL%FID_FEED%MNWLINK) .AND. IUNIT(50) == Z) CALL STOP_ERROR(OUTPUT=FWEL%IOUT, MSG='FMP SUPPLY_WELL HAS WELLS WITH LAY=0, WHICH INDICATES AN MNW2-LINK, BUT THE MNW2 PACKAGE IS NOT AVAILIBLE.')
    !
    ERR = BLNK
    DO FD=ONE, NFEED
      ...
      END IF
    END DO
    IF (ERR .NE. BLNK) CALL STOP_ERROR(OUTPUT=FWEL%IOUT, MSG='FMP SUPPLY_WELL WITH MNW2-LINK FAILED TO LOCATE THE FOLLOWING WELL NAMES IN THE MNW2 WELLID LIST:'//NL//'(NO NAME MATCH BETWEEN THESE FMP FWELLs AND THE LIST OF MNW2 WELLs)'//NL//ERR//NL)
    !
  END SUBROUTINE
  !
  IMPURE ELEMENTAL SUBROUTINE FWELL_BUILD_MNW2_INDEX(FWEL,HAS_MNW2)
    ...
    IF(FWEL%LOAD_TYPE == TWO .AND. FWEL%NF==ONE ) THEN
             DO FD=ONE, SIZE(FWEL%FID_FEED)
                 IF(FWEL%FID_FEED(FD)%MNWLINK .AND. .NOT. HAS_MNW2) CALL STOP_ERROR(OUTPUT=FWEL%IOUT, MSG='FMP SUPPLY_WELL HAS WELLS WITH LAY=0 OR "MNW", WHICH INDICATES AN MNW2-LINK, BUT THE MNW2 PACKAGE IS NOT AVAILIBLE.')
             END DO
             !
             IF(HAS_MNW2) THEN
    ...
    ELSEIF( FWEL%MNWLINK .AND. FWEL%LOAD_TYPE.NE.TWO .AND. FWEL%DIM > Z) THEN
             !
             IF(.NOT. HAS_MNW2) CALL STOP_ERROR(OUTPUT=FWEL%IOUT, MSG='FMP SUPPLY_WELL HAS WELLS WITH LAY=0, WHICH INDICATES AN MNW2-LINK, BUT THE MNW2 PACKAGE IS NOT AVAILIBLE.')
             !
             NF = FWEL%NF
             ERR=BLNK
             DO I=1, FWEL%DIM
...
!    !
!    IF(FWEL%MNWLINK .AND. IU_MNW2 == Z) CALL STOP_ERROR(OUTPUT=FWEL%IOUT, MSG='FMP SUPPLY_WELL HAS WELLS WITH LAY=0, WHICH INDICATES AN MNW2-LINK, BUT THE MNW2 PACKAGE IS NOT AVAILIBLE.')
!    !
!    NF = FWEL%NF
!    ERR=BLNK
!    DO I=1, FWEL%DIM
...
!  END SUBROUTINE
        ...
        WELL: DO I=1, FWEL%DIM
          IF(FWEL%MNWLOC(I) > Z) THEN
                ...
                ELSEIF ( IACT == ONE .AND. .NOT. MASTER_ACT) THEN
                    ...
                    END IF
                    !WRITE(IOUT,'(3A)') 'WARNING: FMP SUPPLY_WELL "',FWEL%WELLID(I),'" WITH MNW-LINK IS NOT SPECIFIED FOR USE DURING THIS STRESS PERIOD, BUT THE MNW2 WELL OF THE SAME NAME IS AVAILIBLE. THE MNW2 WELL WILL HAVE ITS DESIRED PUMPING RATE SET TO ZERO FOR THE STRESS PERIOD (NO PUMPING BUT INTRABOREHOLE FLOW).'
                END IF
                !
  ...
  !!!          CALL WARNING_MESSAGE(OUTPUT=IOUT,MSG='FMP SUPPLY_WELL: FOR FARM '//NUM2STR(FWEL%NF)//NL// &
...
!                NAM_NOMNW = TRIM(NAM_NOMNW)//NL//FWEL%WELLID(I)
!                CYCLE WELL
!                !WRITE(IOUT,'(/3A/)') 'WARNING: FMP SUPPLY_WELL "',FWEL%WELLID(I),'" WITH MNW-LINK POINTS TO AN MNW2 WELL THAT IS INACTIVE (NOT AVAILIBLE) FOR THE CURRENT STRESS PERIOD. THIS FARM WELL WILL BE REMOVED FROM THIS STRESS PERIOD'
!                !
...
!  END SUBROUTINE
  ...
  IMPURE ELEMENTAL SUBROUTINE PRINT_INPUT_TRANSCRIPT(FWEL, KPER, IU2)
    USE GLOBAL,                     ONLY: BOTM, LBOTM, SPSTART
    CLASS(FARM_WELL_DATA), INTENT(INOUT)::FWEL
    ...
    !
    AUX =''
    IF(FWEL%NF == ONE .AND. ((KPER==SPSTART .OR. IU==FWEL%IOUT) .OR. FWEL%IOUT==IU) ) THEN  !PRINT FOR FIRST FARM AND EITHER FIRST SP/TS OR IF WRITING TO LIST FILE
        !
    ...
    !
    IF(ANY( FWEL%LOAD_TYPE == [2,3] ))  ALLOCATE(CHARACTER(768)::FEEDFILE)
  ...
  END SUBROUTINE
  !
  IMPURE ELEMENTAL SUBROUTINE PRINT_SMOOTHED_PUMPING(FWEL, KPER, KSTP, TOTIME, DATE, SET_HEADER)  !ONLY RUN AFTER CALLING CALCULATE_PUMPING_TO_MEET_DEMAND
    USE GLOBAL,        ONLY: HNEW, BOTM, LBOTM
    CLASS(FARM_WELL_DATA), INTENT(INOUT)::FWEL
    ...
    !
    IF(FWEL%NF==ONE .AND. (SET_HEADER .OR. FWEL%IOUT==IU) ) THEN
        !
        IF (FWEL%IOUT==IU) WRITE(IU,*)
        !
  ...
  !END SUBROUTINE
  !
  IMPURE ELEMENTAL SUBROUTINE PRINT_OUT_BYWELL(FWEL, KPER, KSTP, DELT, DYEAR, DATE, SET_HEADER)
    !NOTE THAT THE CALCULATED ALLOTMENT WILL BE OFF WHEN USING NOCIRNOQ FOR WELLS WITH NOCIR
    USE GWFMNW2MODULE, ONLY: MNW2, MNWNOD
    CLASS(FARM_WELL_DATA), INTENT(INOUT)::FWEL
    INTEGER,         INTENT(IN):: KPER, KSTP
    ...
    INTEGER:: I, J, IL, IR, IC, IU, MNW, EXT_FLG, FIRSTNODE, LASTNODE
    DOUBLE PRECISION:: FRAC, SMF, QACT_BIN, QDMD_BIN
    CHARACTER(17):: QINI, QSMF, QACT, QDMD, DT
    CHARACTER(20):: WELLID
    !
    IF(FWEL%OUT_BYWELL%IU == Z) RETURN  !NOTHING TO PRINT OUT
    !
    ...
    !
    IF(FWEL%OUT_BYWELL%BINARY) THEN
                          IF(FWEL%NF == ONE .AND. SET_HEADER) WRITE(FWEL%IOUT,'(A,/A)')'FARM WELL BY WELL OUTPUT WRITTEN TO BINARY FILE USING STREAM UNFORMATTED STRUCTURE. EACH THE RECORD IN BINARY HAS THE FOLLOWING STRUCTURE:','WELLID (20char), LAY (int), ROW (int), COL (int), MNWLINK (int), EXTERNAL_USE_FLAG (int) DATE_START (19char), DECIMAL YEAR (double), TIME STEP LENGTH (double), STRESS PERIOD (int), TIME STEP (int), FARM ID (int), INITIAL WELL CAPACIY (double), WELL ALLOTTED CAPACITY (double), WELL ALLOTTED AND MNW LIMITTED CAPACITY (double), WELL ALLOT, MNW, AND SMOOTHED CAPACITY (double), FINAL WELL PUMPAGE FROM FMP (double), FINAL WELL PUMPAGE FROM MNW2 [SET TO FMP RATE IF NOT LINKED TO MNW] (double)  '
    ELSE
                          IF(FWEL%NF==ONE .AND. (SET_HEADER .OR. FWEL%IOUT==IU) )  THEN
                              !
                              IF (FWEL%IOUT==IU) WRITE(IU,*)
  ...
  END SUBROUTINE
  !
  IMPURE ELEMENTAL SUBROUTINE PRINT_OUT_BYLAYER(FWEL, KPER, KSTP, DELT, DYEAR, DATE, WBS, SET_HEADER) 
  ...
  END SUBROUTINE
  !
  IMPURE ELEMENTAL SUBROUTINE PRINT_OUT_BYMNW(FWEL, KPER, KSTP, DELT, DYEAR, DATE, SET_HEADER)
    !NOTE THAT THE CALCULATED ALLOTMENT WILL BE OFF WHEN USING NOCIRNOQ FOR WELLS WITH NOCIR
    ...
    LOGICAL,         INTENT(IN):: SET_HEADER
    INTEGER:: I, J, IU, FIRSTNODE, LASTNODE
    DOUBLE PRECISION:: FRAC, QACT_BIN
    ...
    IF(FWEL%OUT_BYMNW%BINARY) THEN
                          IF(FWEL%NF == ONE .AND. SET_HEADER) WRITE(FWEL%IOUT,'(A,/A)')'FARM WELL BY MNW2 WELL OUTPUT WRITTEN TO BINARY FILE USING STREAM UNFORMATTED STRUCTURE. EACH THE RECORD IN BINARY HAS THE FOLLOWING STRUCTURE:','WELLID (20char), LAY (int), ROW (int), COL (int), MNWLINK (int), EXTERNAL_USE_FLAG (int) DATE_START (19char), DECIMAL YEAR (double), TIME STEP LENGTH (double), STRESS PERIOD (int), TIME STEP (int), FARM ID (int), INITIAL WELL CAPACIY (double), WELL ALLOTTED CAPACITY (double), WELL ALLOTTED AND MNW LIMITTED CAPACITY (double), WELL ALLOT, MNW, AND SMOOTHED CAPACITY (double), FINAL WELL PUMPAGE FROM FMP (double), FINAL WELL PUMPAGE FROM MNW2 [SET TO FMP RATE IF NOT LINKED TO MNW] (double)  '
    ELSE
                          IF(FWEL%NF==ONE .AND. (SET_HEADER .OR. FWEL%IOUT==IU) )  THEN
                              !
                              IF (FWEL%IOUT==IU) WRITE(IU,*)
  ...
  END SUBROUTINE
  !
  IMPURE ELEMENTAL SUBROUTINE PRINT_OUT_BYFARM(FWEL, KPER, KSTP, DELT, DYEAR, DATE, TFDR_INI, TFDR, SET_HEADER)
    CLASS(FARM_WELL_DATA), INTENT(INOUT)::FWEL
    INTEGER,         INTENT(IN):: KPER, KSTP
    DOUBLE PRECISION,INTENT(IN):: DELT
    DOUBLE PRECISION,INTENT(IN):: DYEAR
    ...
    CHARACTER(17):: QINI, QALOT, QCAP, QFIN, DT, TFINI, TF
    CHARACTER(17):: ZER
    !
    ...
    IF(FWEL%OUT_BYFARM%BINARY) THEN
       IF(FWEL%NF == ONE .AND. SET_HEADER) WRITE(FWEL%IOUT,'(A,/A)')'FARM WELL CAPCITIES BY FARM OUTPUT WRITTEN TO BINARY FILE USING STREAM UNFORMATTED STRUCTURE. EACH THE RECORD IN BINARY HAS THE FOLLOWING STRUCTURE:',"DATE_START (19char), DECIMAL YEAR (double), TIME STEP LENGTH (double), STRESS PERIOD (int), TIME STEP (int), FARM ID (int), INITIAL FARM'S CAPACIY (double), FARM'S CAPACITY (double), FINAL FARM'S PUMPAGE (double), INITIAL FARM'S DEMAND (double)"
       !
       IF(FWEL%N > Z) THEN
...
!######################################################################################################################
!      
    !***SUPERCEDED BY WBS_DATA_FMP_MODULE
