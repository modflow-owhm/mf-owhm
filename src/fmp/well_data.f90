...
!  END SUBROUTINE
  ...
  SUBROUTINE FWEL_LOAD_FID_LINEFEED(BL,FWEL,INSTYLE)
    ...
    DO I=ONE, NFEED
        ...
        IF (FWEL(ONE)%FID_FEED(I)%FL%IS_CONSTANT) CALL STOP_ERROR(BL%LIST%LN, BL%IU, BL%IOUT, 'FMP FWELL BLOCK DOES NOT ALLOW FOR THE KEYWORD "CONSTANT" AFTER THE KEYWORD "LINEFEED WBS". LINEFEED INPUT MUST BE SPECIFIED IN A SPEARATE FILE.') 
        IF (FWEL(ONE)%FID_FEED(I)%FL%IS_INTERNAL) CALL STOP_ERROR(BL%LIST%LN, BL%IU, BL%IOUT, 'FMP FWELL BLOCK DOES NOT ALLOW FOR THE KEYWORD "INTERNAL" AFTER THE KEYWORD "LINEFEED WBS". LINEFEED INPUT MUST BE SPECIFIED IN A SPEARATE FILE.') 
        !
        CALL BL%LIST%NEXT()
        CALL BL%LIST%SET_LN()
    END DO
