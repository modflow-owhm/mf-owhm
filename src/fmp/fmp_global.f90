...
MODULE FMP_GLOBAL
  ...
  USE          WELL_DATA_FMP_MODULE, ONLY:     FARM_WELL_DATA
  USE          SOIL_DATA_FMP_MODULE, ONLY:          SOIL_DATA
  USE          CROP_DATA_FMP_MODULE, ONLY:          CROP_DATA
  USE        OUTPUT_DATA_FMP_MODULE, ONLY:        OUTPUT_DATA
  USE       CLIMATE_DATA_FMP_MODULE, ONLY:       CLIMATE_DATA
  USE       OPTIONS_DATA_FMP_MODULE, ONLY:       OPTIONS_DATA
  USE      SALINITY_DATA_FMP_MODULE, ONLY:      SALINITY_DATA
  ...
  SUBROUTINE FMP3DA(IGRID)
    ...
    !!!DEALLOCATE(SWFL)
    !!!SWFL=>NULL()
    DEALLOCATE(FMPDAT(IGRID)%FDIM)
    DEALLOCATE(FMPDAT(IGRID)%WBS)
    DEALLOCATE(FMPDAT(IGRID)%SFR_DELIV)
    DEALLOCATE(FMPDAT(IGRID)%CLIMATE)
    DEALLOCATE(FMPDAT(IGRID)%ALLOT)
    DEALLOCATE(FMPDAT(IGRID)%FCROP)
    DEALLOCATE(FMPDAT(IGRID)%SOIL)
    DEALLOCATE(FMPDAT(IGRID)%SALT)
    DEALLOCATE(FMPDAT(IGRID)%SWODAT)
    DEALLOCATE(FMPDAT(IGRID)%FMPOPT)
    DEALLOCATE(FMPDAT(IGRID)%FMPOUT)
    DEALLOCATE(FMPDAT(IGRID)%SWFL)
    !
...
MODULE FMPBLK
  ...
  !
  DOUBLE PRECISION,PARAMETER :: AC=1.0D+10 !AC & AR MUST BE THE SAME
  REAL, PARAMETER ::            AR=1.0E+10
  !
  !   FLOW RATE PRECISION (DEPENDING ON LENUNI AND ITMUNI)       
  DOUBLE PRECISION,PARAMETER :: FPS=1.0D-10
  !
  ...
  REAL,PARAMETER :: TPL=1.0E-10 !LOWER
  REAL,PARAMETER :: TPU=1.0E+07 !UPPER
  !     OPTIMIZATION PRECISION IN SIMPLEX ROUTINE
  DOUBLE PRECISION,PARAMETER :: OPS=1.0D-10
  !     OTHER
  DOUBLE PRECISION,PARAMETER :: ZERO=0.0D0
  REAL, PARAMETER ::            ZER=0.0
  !
END MODULE FMPBLK
!
MODULE FMPFUNCT 
  ...
  FUNCTION RTFUNC(D,M,N,Y,XACC,IC,IR)      ! seb VARIABLE J REMOVED, ONLY PROVIDES CONVERGENCE ITERATION, BUT NEVER USED. CAN NOT MAKE PURE ROUTINE BECAUSE OF WRITE(*,*) AND STOP STATMENT
    ...
    DOUBLE PRECISION, PARAMETER:: TOL= 1.D-5                          !PRECISION TO SOLVE ROOT AT
    ...
    IF(Y.GE.D) THEN
        ...
        ELSE
            X1 = ((DLOG(Y/D)/DLOG(M/D))**(1.D30))*(D-UNO)+UNO - XACC
           !X1 = UNO - XACC
        END IF
    END IF
    IF(Y.LT.D) X1=UNO
    IF(X1.LT.UNO) X1=UNO
    IF(N.GE.1.D-30) THEN
      X2 = ((DLOG((Y+D)/D)/DLOG(M/D))**(UNO/N))*(D-UNO)+UNO +XACC
    ELSE
      X2 = ((DLOG((Y+D)/D)/DLOG(M/D))**(1.D30))*(D-UNO)+UNO +XACC
     !X2 = UNO + XACC
    END IF
  ...
  PURE FUNCTION FUNC(X,D,M,N,Y)
    ...
    !
    FUNC = D*((M/D)**(((X-UNO)/(D-UNO))**N))+X-D-Y
    !
  END FUNCTION
!
END MODULE FMPFUNCT
